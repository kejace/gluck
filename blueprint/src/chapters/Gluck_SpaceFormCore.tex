...
\gluckfigure{spaceform-unification}
  {The same periodic intrinsic curvature function $\kappa_K\equiv2$ realized in
   all three space forms. The resulting geodesic circles have metric-dependent
   intrinsic radii: $\operatorname{arccot}2$, $1/2$, and
   $\operatorname{arccoth}2$. Their displayed chart radii therefore differ as
   well. Numbered points are quarter-period samples, not curvature extrema.}
  {A constant periodic curvature graph appears above a small circle on a
   sphere, a Euclidean circle, and a smaller circle in the Poincare disk. Each
   realization has four numbered quarter-period samples and a different radius.}
