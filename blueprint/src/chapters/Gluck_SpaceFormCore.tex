...
\gluckfigure{spaceform-unification}
  {Three analogous nonconstant four-vertex reconstructions, one in each space
   form. Each periodic curvature profile is matched only to the curve beneath
   it; the figure does not assert that the three profile functions are equal.
   The sphere and Poincaré disk make the different ambient metrics explicit.}
  {Three columns pair periodic nonconstant curvature graphs with a curve on a
   sphere, a Euclidean oval, and a curve inside the Poincare disk. Four numbered
   extrema correspond to four numbered curve points within each column.}
