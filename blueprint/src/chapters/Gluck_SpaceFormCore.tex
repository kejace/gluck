...
% parameter ε ∈ {+1, −1, 0} (ambient curvature K = ε): ε = +1 recovers S²
% verbatim, ε = −1 is the Poincaré-disk model of H², and ε = 0 is the flat
...
\{+1,-1,0\}$. All three constant-curvature models live in the open unit disk
$\{|z|<1\}\subseteq\mathbb C$ in the tangent-angle gauge: the sphere
$S^2$ ($K=+1$, round metric $4/(1+|z|^2)^2$) is $\varepsilon=+1$, the
hyperbolic plane $H^2$ ($K=-1$, Poincar\'e metric $4/(1-|z|^2)^2$) is
$\varepsilon=-1$, and the Euclidean plane $E^2$ ($K=0$, flat conformal
metric $\lambda=2$, so that the disk-gauge curvature is half the Euclidean
one) is $\varepsilon=0$. The single unifying object is the \emph{space-form
gauge speed}
\[
...
\item \emph{The confinement threshold.} Where the admissibility lower bound
  ...
  uniform threshold $(1-\varepsilon)/2$: for $\varepsilon=+1$ it is $0$
  (positivity of $\kappa$ suffices), for $\varepsilon=-1$ it is $1$ (the
  escape-velocity bound — geodesic circles in $H^2$ have curvature
  $\coth\rho>1$, and curves with $\kappa\le1$ never close), and for
  $\varepsilon=0$ it is $1/2$ (the Euclidean circle of disk-gauge curvature
...
\item \emph{No absolute floor on the bracket scale.} The bracket value at the
  ...
  $\varepsilon=0$ the coefficient \emph{vanishes} — the flat gauge field is
...
[space-form four-vertex hypothesis]
  ...
  $\varepsilon\le0$ — the uniform confinement floor
  $\kappa(\theta)>(1-\varepsilon)/2$ for all $\theta$. At $\varepsilon=+1$
  the last clause is vacuous and this is \cref{def:sphere_four_vertex} (with
  a trivially true extra conjunct); at $\varepsilon=-1$ it is the
  escape-velocity bound $\kappa>1$, the threshold below which curves in
  $H^2$ cannot close; at $\varepsilon=0$ it is the flat threshold
...
[space-form centered radius]
  ...
  \cref{lem:centered_radius_pos} ($\varepsilon=1$), the hyperbolic radius
  $c-\sqrt{c^2-1}$ ($\varepsilon=-1$, for $c>1$), and — unlike the legacy
  unrationalized form $\varepsilon(\sqrt{c^2+\varepsilon}-c)$, which
  vanishes identically at $\varepsilon=0$ — the flat radius $1/(2c)$
  ($\varepsilon=0$, for $c>1/2$). The two forms agree at
  $\varepsilon=\pm1$ (\texttt{centeredRadius\_eq}).
\end{definition}
...
[centered-radius algebra]
  ...
  For $\varepsilon\in\{+1,-1,0\}$ and an admissible level $c$ (meaning
  $c>0$ when $\varepsilon=1$, $c>1$ when $\varepsilon=-1$, $c>1/2$ when
  $\varepsilon=0$), the centered radius
...
\begin{proof}
  ...
  clear the denominator $\sqrt{c^2+\varepsilon}+c>1$ (positive in all three
  admissible windows, \texttt{window\_pos}) and use
  $(\sqrt{c^2+\varepsilon})^2=c^2+\varepsilon$, then
  \texttt{linear\_combination}/\texttt{nlinarith}; the membership bounds
  compare $\sqrt{c^2+\varepsilon}$ against $c$ and the threshold by strict
  monotonicity of the square root, and the threshold identity is a per-sign
  case split (at $\varepsilon=0$, $r^\ast=1/(2c)<1\iff c>1/2$).
...
[uniform step-model margins near the centered circle]
  ...
  For $\varepsilon\in\{+1,-1,0\}$, an admissible level $c$, and any
...
[first-variation expansion of the step error map]
  ...
  $\eta(\varepsilon)$. The $\varepsilon$ factor is mandatory — it descends
  from the $\varepsilon$ on the right-hand side of
  \cref{lem:spaceform_speed_sub_radius} — so $\eta>0$ for the sphere,
  $\eta<0$ for the hyperbolic plane, and $\eta=0$ for the flat plane
  ($\varepsilon=0$, where the four-arc error map is identically zero and the
...
[nonvanishing first-variation coefficient]
  ...
  For $\varepsilon\in\{+1,-1\}$ (genuinely two-sign: the flat coefficient
  vanishes),
