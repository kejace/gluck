...
\begin{lemma}[two-solution uniqueness on a subinterval]
...
\begin{proof}
  ...
  reusable; only \cref{thm:existence_of_zero} is. \emph{Caveat for the formalization:}
  \cref{lem:invariant_admissible_domain} takes $\kappa^\ast$ \emph{continuous}, while the
  step model is piecewise constant — compare through continuous approximants of the step,
  or restate the transport with $z^\ast$ merely continuous piecewise-$C^1$ satisfying the
  integral identity $z^\ast(\theta)=z^\ast(0)+\int_0^\theta F_{\kappa^\ast}(s,z^\ast(s))ds$
  (which concatenated arcs satisfy); decide when S2-D is dispatched.
\end{proof}
