...
\begin{lemma}[speed dominates the centered radius]
...
\begin{lemma}[exact level sensitivity of the gauge speed]
...
\begin{lemma}[four-arc composite form of the step error map]
...
\begin{definition}[quarter-arc margin package]
...
\begin{lemma}[one quarter-arc of the step transport]
...
\begin{lemma}[uniform margins of the step model near the centered circle]
  ...
  $z_0^\ast=-i\,r^\ast$. There exist explicit $0<R<1$ (the positivity witness is needed
...
\begin{definition}[conjugate boundary loop]
  \label{def:conj_loop}
  \lean{Gluck.conjLoop}
...
\begin{lemma}[the conjugate loop avoids the origin]
...
\begin{lemma}[trajectory speed: continuity, periodicity, positivity]
  ...
  \uses{def:spherical_speed, def:periodic_extension, lem:periodic_extension_periodic,
        lem:reconstruction_ode, def:is_curvature_function}
  Let $\kappa$ be a curvature function, $R<1$, $\delta>0$, and let
...
\begin{lemma}[the closing trajectory is a translated reconstruction curve]
...
\begin{lemma}[simplicity is translation-invariant]
...
\begin{lemma}[simplicity]
  ...
  In the setting of \cref{lem:spherical_trajectory_speed} (curvature function $\kappa$,
  $R<1$, $\delta>0$, an admissible closed trajectory $z$ of the truncated flow on
