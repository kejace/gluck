...
[spherical converse, positive stage]
  ...
  \uses{def:realizes_spherical_curvature, def:sphere_four_vertex,
        ...
        lem:invariant_admissible_domain, lem:spherical_endpoint_winding,
...
\begin{proof}
  \uses{lem:reconstruction_ode, lem:invariant_admissible_domain,
        lem:spherical_endpoint_winding, lem:spherical_simplicity,
  ...
  branch, assemble the proof arc of \cref{sec:sphere_arc}.
  \cref{lem:spherical_endpoint_winding} supplies a closing configuration $z_0^\ast$ with
  $E(z_0^\ast)=0$, i.e.\ the reconstruction trajectory of \cref{lem:reconstruction_ode}
  through $z_0^\ast$ is a closed $C^1$ curve; it is confined to $\{|z|\le R'\}\subset
  \{|z|<1\}$ by \cref{lem:invariant_admissible_domain}, and it realizes $\kappa$ in the
  gauge $\varphi(\theta)=\theta$ because the ODE field is exactly the algebraically solved
  speed relation of \cref{def:realizes_spherical_curvature}
  (\cref{lem:spherical_speed_solve}). \cref{lem:spherical_simplicity} gives simplicity in
  the disk, hence on $S^2$. This mirrors how \texttt{dahlbergConverse} assembles its
  closing parameter, simplicity transport, and reconstruction. \emph{(Assembled formally in
  S2-E once the arc lemmas exist in Lean.)}
\end{proof}
...
\begin{lemma}[field norm equals speed]
...
\begin{lemma}[field Lipschitz bound]
...
\begin{lemma}[field joint continuity]
...
\begin{lemma}[curvature sensitivity of the truncated speed]
...
\begin{lemma}[combined field sensitivity]
  ...
  Let $0\le R$, $0<\delta$, $M=\tfrac{1+R^2}{2\delta^2}$, and let $L$ be the Lipschitz
  constant of \cref{lem:truncated_field_lipschitz}. Then for all $\kappa,\kappa^\ast$,
  $\theta$, $z$, $\tilde z$,
  \[
...
\begin{lemma}[Gr\"onwall with $L^1$ drive]
  ...
  then
  \[
    \sup_{t\in[0,T]}d(t)\;\le\;e^{LT}\Bigl(d_0+\int_0^T g\Bigr) .
  \]
...
[invariant admissible domain — perturbative margin transport]
  ...
  \]
  Let $L$ be the Lipschitz constant of \cref{lem:truncated_speed_lipschitz} and
...
[truncation removal: admissible closed trajectories realize $\kappa$]
  ...
  \uses{lem:truncated_speed_eq, lem:spherical_flow_spec, lem:truncated_speed_pos,
        ...
        lem:spherical_speed_periodic}
  Let $\kappa$ be a curvature function, $0<\delta$, $0\le R<1$, and let
  ...
  for all $\theta$) and closed ($z(2\pi)=z(0)$). Then the $2\pi$-periodic extension of $z$
  is a $C^1$ closed curve confined to the open unit disk which solves the \emph{true}
  reconstruction ODE $z'=q_\kappa(\theta,z)e^{i\theta}$ and realizes $\kappa$
  (\cref{def:realizes_spherical_curvature}) in the gauge $\varphi(\theta)=\theta$.
\end{lemma}
...
[constant-curvature arcs are explicit circular arcs]
  ...
  \uses{def:spherical_speed, lem:spherical_flow_unique}
...
[half-turn equivariance]
  ...
  \uses{def:truncated_speed, lem:spherical_flow_unique}
  ...
  $z(\cdot)$ is a trajectory then so is $y(\theta)=-z(\theta+\pi)$. Consequently, writing
  $H$ for the half-period map $z_0\mapsto\Phi(z_0,\pi)$ of a $\pi$-periodic curvature: if
  $H(z_0)=-z_0$ then the trajectory through $z_0$ closes at $2\pi$ with the central
  symmetry $z(\theta+\pi)=-z(\theta)$.
\end{lemma}

\begin{proof}
  $\|{-z}\|=\|z\|$ and $\langle -z,\,ie^{i(\theta+\pi)}\rangle=\langle -z,\,-ie^{i\theta}
  \rangle=\langle z,ie^{i\theta}\rangle$, and $\kappa(\theta+\pi)=\kappa(\theta)$; so
  every ingredient of $\hat q$ is unchanged. Then
  $y'(\theta)=-z'(\theta+\pi)=-\hat q\,e^{i(\theta+\pi)}=\hat q(\theta,y(\theta))e^{i\theta}$,
  a trajectory. If $H(z_0)=-z_0$, then $y(0)=-z(\pi)=z_0=z(0)$, so $y=z$ by uniqueness
  (\cref{lem:spherical_flow_unique}), i.e.\ $z(\theta+\pi)=-z(\theta)$; at $\theta=\pi$
  this gives $z(2\pi)=-z(\pi)=z_0$.
\end{proof}

\begin{lemma}
[spherical endpoint map: boundary winding over the symmetric step model]
  ...
  \uses{def:spherical_endpoint, lem:spherical_endpoint_continuousOn,
        lem:constant_curvature_arc, lem:flow_half_turn_equivariance,
        lem:invariant_admissible_domain, thm:existence_of_zero}
  For $\kappa$ obtained from the four-vertex reparametrization ($L^1$-close to a
  two-value step model with values $a>b>0$), there is a compact disk $D$ of initial
  points $z_0$, centered at the model's closing configuration, on which the endpoint
  map $E$ of \cref{def:spherical_endpoint} is continuous, nonzero on $\partial D$, and
  has nonzero boundary winding; the in-tree \cref{thm:existence_of_zero} then supplies an
  interior zero $E(z_0^\ast)=0$, a closed trajectory of the truncated flow.
\end{lemma}

\begin{proof}
  \emph{(Pinned design, two sub-questions still open — S2-D is the remaining hard phase.)}
  \textbf{DO-NOT (degeneracy of the constant model).} For \emph{constant} $\kappa=K$
  every admissible initial point lies on a spherical circle of curvature $K$
  (\cref{lem:constant_curvature_arc}), so every trajectory closes and $E\equiv0$ on an
  open set: the constant-curvature family carries no degree and must NOT be used as the
  homotopy target. Equivalently, the $z$-gradient of $q$ vanishes identically along
  circle solutions (a consequence of the consistency identity), so the linearized
  monodromy is the identity.
  \textbf{Model.} Use instead the \emph{symmetric step bicircle}: the two-value
  $\pi$-periodic step curvature (values $a,b$, the in-tree canonical breakpoint pattern),
  whose trajectories obey the half-turn equivariance
  (\cref{lem:flow_half_turn_equivariance}). Closure reduces to the 2-D fixed-point
  equation $G(z_0):=H(z_0)+z_0=0$ for the half-period map $H$. At $a=b=K$ the map $G$
  has the explicit zero $z_0=-i\,r^\ast$, $r^\ast=\sqrt{1+K^2}-K$ (the centered circle),
  and — because the linearization along circle arcs is the identity — $DG=2\,\mathrm{Id}$
  there: an isolated nondegenerate zero with local winding $+1$. This is where the
  linearization degeneracy becomes a \emph{feature}.
  \textbf{Quantitative scheme (strategy-critic refinement, accepted).} The two step
  levels $a>b$ are \emph{free} inside the four-vertex overlap interval
  $(\max(\kappa(q_1),\kappa(q_2)),\,\min(\kappa(p_1),\kappa(p_2)))$, so they may be
  chosen with \emph{small contrast} $h=a-b$ around a midpoint $c$. Then the step model
  is an $O(h)$ perturbation of the constant $c$, the continuation from the explicit
  $a=b$ zero is \emph{local} (boundary nonvanishing on a fixed small disk from
  $DG=2\,\mathrm{Id}+O(h)$), and — crucially — $H$ for the step model is an
  \emph{explicit closed-form} composition of two arc maps
  $z_0\mapsto z_0+i\,q_K(\theta_0,z_0)\,e^{i\theta_0}(1-e^{i\Delta})$
  (\cref{lem:constant_curvature_arc}), so $DH$ and all isolation radii are finite
  symbolic computations, with no variational-equation formalization.
  \textbf{Open sub-question (a) — sharpened.} Compute $DG=DH+\mathrm{Id}$ for the
  explicit two-arc composition and verify invertibility (hence local winding $+1$) for
  small $h>0$ at the symmetric configuration.
  \textbf{Open sub-question (b) — sharpened.} Transfer the winding from $G$ to the
  full-period $E$. Note $E(z_0)=-H_2(-H_1(z_0))-z_0$ with $H_1=H_2=H$ in the
  $\pi$-periodic case, so $DE=DH^2-\mathrm{Id}=O(h)$: the model zero of $E$ is isolated
  only at scale $h$ (for $h=0$, $E\equiv0$ — the DO-NOT above). The order of quantifiers
  works: first fix the contrast $h$, extract the $O(h)$ isolation radius and boundary
  margin of $E$ from the explicit $DE$, THEN choose the reparametrization quality
  $\varepsilon\ll h$ (the $L^1$ distance of \cref{chap:dahlberg_step1} is free) so the
  $\kappa$-homotopy (flow $L^1$-continuity, the estimate of
  \cref{lem:invariant_admissible_domain}) keeps $E\ne0$ on the boundary circle
  throughout. The Euclidean \texttt{errorMap\_winding\_eq\_one} \emph{value} is not
  reusable; only \cref{thm:existence_of_zero} is.
\end{proof}
