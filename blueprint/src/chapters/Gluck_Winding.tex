...
% ---------------------------------------------------------------------------
% Additive layer @084 (discrete program, D-R2-pos): explicit winding values for
% exponential and nonsingular real-linear loops, and the PUBLIC homotopy
% wrapper.  ADDITIVE ONLY -- no existing declaration in Gluck/Winding.lean may
% change (frozen file; all four are new public theorems).  Consumers:
% lem:closure_winding_t0 and lem:closure_persists in
% chapters/Gluck_Discrete_Closing.tex.
% ---------------------------------------------------------------------------

\begin{lemma}[Winding of a scaled exponential loop]
  \label{lem:winding_number_c_exp_loop}
  \lean{Gluck.windingNumberC_expLoop}
  \uses{def:winding_number_c, def:norm_loop, lem:winding_number_eq_div_of_lift,
        lem:winding_number_c_const_mul}
  Project-bespoke. For $c \in \mathbb{C}$, $c \ne 0$, the loop
  $\gamma(t) = c\,e^{2\pi i t}$ ($t \in [0,1]$) is nowhere zero and
  $W_{\mathbb{C}}(\gamma) = 1$.
\end{lemma}
\begin{proof}
  \uses{lem:winding_number_eq_div_of_lift, lem:winding_number_c_const_mul}
  Nowhere-vanishing is $|c\,e^{2\pi i t}| = |c| > 0$. By
  \cref{lem:winding_number_c_const_mul} it suffices to treat $c = 1$. The
  normalised loop of $t \mapsto e^{2\pi i t}$ is itself, with continuous angle
  lift $\varphi(t) = 2\pi t$ (i.e.\ $\mathrm{Circle.exp}(\varphi(t))$ equals the
  loop pointwise); by \cref{lem:winding_number_eq_div_of_lift} the winding is
  $(\varphi(1) - \varphi(0))/2\pi = 1$.
\end{proof}

\begin{lemma}[Winding of a reversed scaled exponential loop]
  \label{lem:winding_number_c_exp_loop_rev}
  \lean{Gluck.windingNumberC_expLoopRev}
  \uses{def:winding_number_c, def:norm_loop, lem:winding_number_eq_div_of_lift,
        lem:winding_number_c_const_mul}
  Project-bespoke. For $c \in \mathbb{C}$, $c \ne 0$, the loop
  $\gamma(t) = c\,e^{-2\pi i t}$ ($t \in [0,1]$) is nowhere zero and
...
\begin{proof}
  \uses{lem:winding_number_eq_div_of_lift, lem:winding_number_c_const_mul}
  As in \cref{lem:winding_number_c_exp_loop} with the explicit lift
  $\varphi(t) = -2\pi t$: the winding is $(\varphi(1) - \varphi(0))/2\pi = -1$.
\end{proof}

\begin{lemma}[Winding of a nonsingular real-linear loop]
  \label{lem:winding_number_c_linear_loop}
  \lean{Gluck.windingNumberC_linearLoop}
  \uses{lem:winding_number_c_exp_loop, lem:winding_number_c_exp_loop_rev,
        lem:winding_number_c_perturb}
  Project-bespoke. Let $a, b \in \mathbb{C}$ with $\lVert a\rVert \ne
  \lVert b\rVert$. The loop
  $\gamma(t) = a\,e^{2\pi i t} + b\,e^{-2\pi i t}$ ($t \in [0,1]$) is nowhere
  zero (reverse triangle inequality: $\lVert\gamma(t)\rVert \ge
  \bigl|\lVert a\rVert - \lVert b\rVert\bigr| > 0$), it is a loop
  ($\gamma(0) = \gamma(1) = a + b$), and
  \[
    W_{\mathbb{C}}(\gamma) =
    \begin{cases} 1 & \lVert b\rVert < \lVert a\rVert,\\
                 -1 & \lVert a\rVert < \lVert b\rVert. \end{cases}
  \]
  (This is the boundary loop of the real-linear map $z \mapsto a z + b\bar z$
  on the unit circle; $\lVert a\rVert^2 - \lVert b\rVert^2$ is its
  determinant.)
\end{lemma}
\begin{proof}
  \uses{lem:winding_number_c_exp_loop, lem:winding_number_c_exp_loop_rev,
        lem:winding_number_c_perturb}
  Case $\lVert b\rVert < \lVert a\rVert$: $\gamma$ is a pointwise perturbation
  of the base loop $t \mapsto a\,e^{2\pi i t}$ by $b\,e^{-2\pi i t}$, of norm
  $\lVert b\rVert < \lVert a\rVert$ = the base norm; both are loops, so
  \cref{lem:winding_number_c_perturb} gives
  $W_{\mathbb{C}}(\gamma) = W_{\mathbb{C}}(t \mapsto a\,e^{2\pi i t}) = 1$ by
  \cref{lem:winding_number_c_exp_loop}. Case $\lVert a\rVert < \lVert b\rVert$
  is symmetric with base $t \mapsto b\,e^{-2\pi i t}$ and
  \cref{lem:winding_number_c_exp_loop_rev}: $W_{\mathbb{C}}(\gamma) = -1$.
\end{proof}

\begin{theorem}[Public homotopy invariance of the $\mathbb{C}$-winding number]
  \label{thm:winding_number_c_homotopy}
  \lean{Gluck.windingNumberC_eq_of_homotopy}
  \uses{def:winding_number_c, def:norm_loop, lem:winding_number_eq_of_homotopy}
  Project-bespoke (the public wrapper P1 for the private
  \cref{lem:winding_number_eq_of_homotopy}). Let $\gamma_0, \gamma_1 \in
  C([0,1], \mathbb{C})$ be nowhere-zero loops and $H \in C([0,1]^2, \mathbb{C})$
  a \emph{nowhere-zero} homotopy through loops: $H(0,\cdot) = \gamma_0$,
  $H(1,\cdot) = \gamma_1$, and $H(s,0) = H(s,1)$ for all $s$. Then
  $W_{\mathbb{C}}(\gamma_0) = W_{\mathbb{C}}(\gamma_1)$.
\end{theorem}
\begin{proof}
  \uses{lem:winding_number_eq_of_homotopy}
  Normalise $H$ pointwise onto $S^1$ by radial projection (nowhere-zero makes
  this continuous, exactly as in the proof of
  \cref{lem:winding_number_c_perturb}); the endpoint slices normalise to the
  normalised loops of $\gamma_0, \gamma_1$ and the loop condition is preserved,
  so \cref{lem:winding_number_eq_of_homotopy} applies and the circle windings
  --- which are by definition the $\mathbb{C}$-windings --- agree.
\end{proof}
