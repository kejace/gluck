...
\gluckfigure{winding-angle-lift}
  {A loop in $\mathbb C^\times$, its normalized circle loop, and a real angle
   lift. The lift gains $4\pi$, so the displayed loop has winding number two.}
  {Three panels show a complex loop around the origin, its normalization on
   the unit circle, and an angle graph rising from zero to four pi.}
  {fig:winding_angle_lift}
...
\end{theorem}
\gluckfigure{winding-error-map}
  {The configuration-to-error map. Its boundary loop winds around the origin,
   forcing an interior parameter with zero endpoint error and hence a closed
   reconstruction.}
  {A parameter disk maps to an error plane whose boundary image encircles the
   origin; sample endpoint defects shrink to zero at an interior parameter.}
  {fig:winding_error_map}
\begin{proof}
  \uses{lem:error_map_winds_boundary}
  \leanok
