...
% archon:covers Gluck/StepReduction.lean
...
\leanok
[Step curvature is bounded below by $a$]
  \label{lem:step_curvature_ge}
  \lean{Gluck.stepCurvature_ge}
  \uses{def:four_arc_step_function}
  If $a \le b$ then $a \le \kappa_0(\theta)$ for all $\theta$; in particular for
  $0 < a < b$ the step curvature is strictly positive.
\end{lemma}
\begin{proof}
  \uses{def:four_arc_step_function}
  \leanok
  $\kappa_0$ takes only the two values $a$ and $b$, and $a \le b$, so every value
  is $\ge a$.
\end{proof}
...
\section{The preliminary reparametrization $h_1$ (project-bespoke construction)}
...
[Calibrated continuous plateau density]
  ...
  \lean{Gluck.exists_plateau_density, Gluck.integral_affine, Gluck.tentBump,
        Gluck.tentBump_nonneg, Gluck.tentBump_continuous, Gluck.tentBump_periodic,
        Gluck.cos_le_cos_half, Gluck.cos_le_cos_half_shift,
        Gluck.tentBump_eq_zero_of_cos_le, Gluck.tentBump_affine_zero,
        Gluck.tentBump_integral_right, Gluck.tentBump_integral_left,
        Gluck.tentBump_integral_center, Gluck.tentBump_integral_support,
        Gluck.tentBump_integral_zero_of_forall, Gluck.tentBump_integral_full,
        Gluck.tentBump_integral_none, Gluck.tentBump_integral_boundary,
        Gluck.tentBump_integral_period, Gluck.tentBump_integral_two_pi}
