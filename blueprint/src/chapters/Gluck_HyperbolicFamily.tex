\chapter{Fork A: the symbolic \texorpdfstring{$(a,c)$}{(a,c)}-family and the up-to-reparam capstone}
...
% chosen per κ, culminating in dahlberg_converse_reparam_deg1:
...
the geodesic curvature of a simple closed curve in $H^2$ ($K=-1$).
...
\item \textbf{A4 (Anchor).}  The anchor confinement estimates: the explicit
  confinement radius $R(a,c)<1$ and the per-arc whole-circle escape bounds.
...
[the anchor confinement radius]
  ...
  The explicit radius $R(a,c)=1-(a-1)(c-1)/(20c^2)<1$ confining the clean
  bicircle arcs strictly inside the Poincaré disk: on the anchor window
  ($h\ge1/(10c)$) each level-$K$ model arc ($K\in\{a,c\}$, $K>1$) obeys the
  square-root-free whole-circle escape bound
  $\|\gamma(\sigma)\|\le1-r(K-1)\le R(a,c)$
  (\texttt{arcModelConst\_norm\_le\_one\_sub\_radius\_mul},
  \texttt{anchor\_arc2\_confined}), from the centre-norm identity
  $\|z_c\|^2=1+r^2-2rK$.  The per-leg escape margin $m_0$ feeds the
  clean-layout confinement of \cref{def:layout_flow} and the chord margin
...
giving the three degrees of freedom the closing needs ($2$ for $\gamma$-closure
$+$ $1$ for turning).
...
[the true layout flow]
  ...
  \uses{def:kappa_arc, def:anchor_confine_radius}
...
[ALM-A6: global confinement of the true flow]
  ...
  $\|\gamma_{\mathrm{true}}\|\le R_{\mathrm{clean}}+b\le R'<1$ there; in
...
[the layout closure residual]
  ...
  $\mathcal R(w_1,w_2,t)
    =\Phi_{\mathrm{true}}(\Lambda)-\bigl(\gamma(0),\varphi(0)+2\pi\bigr)$:
  endpoint state minus the one-full-turn closure target.  The first component
  is the $\gamma$-closure residual (A10 consumes its real and imaginary parts);
...
\begin{definition}\leanok
[the clean $\gamma$-closure residual]
...
[ALM-A10: the true flow closes]
  ...
  the clean curve and the confinement $\|\gamma(\sigma)\|\le R'<1$ on the closed
...
\begin{proof}
  ...
  the $\gamma$-residual of the \emph{true} flow inherits the clean face signs with
  ...
  vanish; invertibility ($AB'-BA'\ne0$) recovers $\gamma$-closure, $\tau$ the
...
[ALM-A12: window-solution exposure]
  ...
  $\gamma:\mathbb R\to\mathbb C$, $\varphi:\mathbb R\to\mathbb R$ with
  $\gamma'=e^{i\varphi}$, $\varphi'$ the arc-length angular speed of
  $\kappa_{\mathrm{arc}}$, $\|\gamma\|<1$, $\gamma(\Lambda)=\gamma(0)$,
...
[capstone: hyperbolic mixed converse, up to a degree-one reparametrization]
  ...
  \lean{Gluck.Hyperbolic.dahlberg_converse_reparam_deg1}
  \uses{def:mixed_sign_hyperbolic_four_vertex, def:spaceform_realizes,
        ...
        lem:layout_window_solution, def:arclength_h2_curvature,
        lem:node_map_add_period,
  ...
  \[
    ...
    \texttt{IsSimpleClosed}\;\gamma,\quad
    \texttt{Realizes}\,(-1)\;\gamma\;(\kappa\circ\Psi).
  \]
...
\begin{proof}
  \uses{lem:exists_abab_levels, thm:exists_bicircle_anchor,
        ...
        lem:layout_chord_ne_zero, lem:layout_window_solution,
  ...
  $\texttt{ArcLengthH2Curvature}\,(\kappa\circ h_1\circ g_{w^*,t^*})$ at
  window $\Lambda=\texttt{nodePeriod}$ by packaging the window-solution
  exposure (\cref{lem:layout_window_solution}) into
  \cref{def:arclength_h2_curvature}.  The arc-length converse
...
[up-to-reparam form, degree-one datum forgotten]
  ...
  \lean{Gluck.Hyperbolic.dahlberg_converse_reparam}
