...
\gluckfigure{hyperbolic-simplicity}
  {A proper sub-arc chord is the integral of its unit tangent and remains
   nonzero under the transported margin. A self-intersection would make such a
   chord vanish, so chord nonvanishing proves simplicity.}
  {A simple curve in the Poincaré disk has two marked points joined by a
   nonzero chord; a crossed curve beside it illustrates the excluded case.}
  {fig:hyperbolic_simplicity}
