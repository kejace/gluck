...
\hline
$\mathbb{E}^2$ & \texttt{Gluck.gluck\_converse} & \texttt{Gluck.dahlbergConverse} \\
$\mathbb{S}^2$ & \texttt{Gluck.sphericalConverse\_pos} & \texttt{Gluck.sphericalConverse} \\
$\mathbb{H}^2$ & \texttt{Gluck.hyperbolicConverse\_pos} & \texttt{Gluck.hyperbolicMixedConverse\_exact} \\
\end{tabular}
\end{center}

All six conclusions are \emph{exact}: the witness curve realizes $\kappa$ on
the nose, with no reparametrization. In addition the $\varepsilon$-generic
space-form layer states the unified capstones
\texttt{Gluck.SpaceForm.spaceFormConverse\_pos} and
\texttt{Gluck.SpaceForm.spaceFormMixedConverse} (with the $\mathbb{H}^2$
instance \texttt{Gluck.SpaceForm.hyperbolicDahlbergConverse}), and the
Euclidean development also proves the arc-length converse
\texttt{Gluck.arcLengthConverse}.
...
\item the \emph{SpaceForm} layer (Part~4) is the spherical flow engine
  transported to a curvature parameter $\varepsilon\in\{+1,-1\}$, re-deriving
...
\item the mixed space-form capstone
  \texttt{Gluck.SpaceForm.spaceFormMixedConverse} \emph{assembles} the
