...
\gluckfigure{euclidean-gluck-reconstructions}
  {Three sample curvature functions in the Euclidean Gluck setting and their
   inclination-parametrized reconstructions. In every column the positive
   radius-of-curvature weight $\rho=1/\kappa$ has no first Fourier harmonic, so
   the resulting convex curve closes. Four selected numbered curvature extrema are
   matched to their corresponding vertices on each curve.}
  {Three columns pair graphs of positive periodic curvature functions with their
   reconstructed plane curves: an oval, an asymmetric mixed-mode curve, and a
   two-frequency harmonic blend. Four selected numbered points in each graph mark local
   curvature extrema, with the same numbers marking the corresponding points on
   the curve. Arrows show the counterclockwise orientation.}
  {fig:euclidean_gluck_reconstructions}
