...
\texttt{dahlberg\_converse}) is complete and is reused, not disturbed. The positive-stage
capstone is named \texttt{spherical\_gluck\_converse}; the name \texttt{spherical\_dahlberg\_converse} is
...
[realizes spherical curvature]
  ...
  A curve $\gamma:\mathbb R\to\mathbb C$ \emph{realizes the spherical curvature function}
  $\kappa:\mathbb R\to\mathbb R$ when it is $C^1$, regular ($\gamma'(t)\neq 0$ for all $t$),
  confined to the open disk ($\|\gamma(t)\|<1$ for all $t$), and there is a differentiable
  tangent-angle function $\varphi:\mathbb R\to\mathbb R$ with, for all $t$,
  \[
    \gamma'(t)=\|\gamma'(t)\|\,e^{i\varphi(t)},\qquad
    \frac{1+\|\gamma(t)\|^2}{2}\,\varphi'(t)
      =\bigl(\kappa(t)-\langle \gamma(t),\,i\,e^{i\varphi(t)}\rangle_{\mathbb R}\bigr)\,\|\gamma'(t)\| .
  \]
  ...
  says precisely that $\kappa$ is the spherical geodesic curvature of $\gamma$. It mirrors the
  Euclidean \cref{def:realizes_curvature} exactly: it is \emph{speed-free} (only the angle
  $\varphi$, not $\gamma$, must be differentiable; the speed $\|\gamma'\|$ is never normalized to arc
...
[spherical converse, positive stage]
  ...
  (\cref{def:sphere_four_vertex}), then there is a simple closed curve $\gamma$ confined to the
  open disk that realizes $\kappa$ as its spherical geodesic curvature:
  \[
    \texttt{SphereFourVertex }\kappa \;\Longrightarrow\;
    \exists\,\gamma,\ \texttt{IsSimpleClosed }\gamma\ \wedge\ \texttt{RealizesSphericalCurvature }\gamma\,\kappa .
  \]
...
\begin{proof}
  \uses{lem:reconstruction_ode, thm:spaceform_endpoint_winding,
        lem:spherical_simplicity, lem:spherical_circle_realizes,
  ...
  (non-constant) branch: \cref{thm:spaceform_endpoint_winding} supplies $R<1$,
  $\delta>0$, a circle reparametrization $h_1$, and a \emph{closed, admissible}
  trajectory $\gamma$ of the $(\kappa\circ h_1,R,\delta)$-truncated flow.
  ...
  the disk, hence on $S^2$. This mirrors how \texttt{dahlberg\_converse} assembles its
...
\item \textbf{S2-D (endpoint map and winding).} The degree argument runs over a compact
  ...
  — see the DO-NOT recorded in \cref{thm:spaceform_endpoint_winding}.
\end{enumerate}
...
\cref{thm:spaceform_endpoint_winding}, the S2-E simplicity chain, and the capstone.
...
[gauge speed]
  ...
  relation of \cref{def:realizes_spherical_curvature} for the speed $\|\gamma'\|$. It is a total
