...
\gluckfigure{sphere-reconstructions}
  {Three inverse-stereographic lifts drawn on $S^2$: two have strictly positive
   spherical geodesic curvature, while the third is in the mixed-sign regime.
   Numbered markers match four selected curvature extrema to the corresponding
   curve points; higher-frequency profiles may have additional vertices.}
  {Three columns pair spherical geodesic-curvature graphs with their smooth
   closed stereographic plane representatives. Two graphs remain positive and
   one crosses zero. Four numbered extrema correspond to identically numbered
   points on each curve.}
  {fig:spherical_reconstructions}
