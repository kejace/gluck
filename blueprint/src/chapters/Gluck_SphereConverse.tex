...
\begin{lemma}[constant curvature branch: the centered circle realizes constant $\kappa$]
  ...
  For every constant $c>0$ the centered circle $\gamma(\theta)=-r^\ast\,i\,e^{i\theta}$ with
...
\begin{proof}
  ...
  \leanok
  $\gamma$ is $C^1$ (an explicit exponential), $2\pi$-periodic, and
  $\gamma'(\theta)=r^\ast e^{i\theta}\ne0$, so the tangent-angle witness is
  $\varphi(\theta)=\theta$ with $\|\gamma'\|=r^\ast$. Confinement: $\|\gamma\|=r^\ast<1$ since
  $\sqrt{1+c^2}<1+c$. The speed relation is the geodesic-circle identity: with
  $\langle \gamma,ie^{i\theta}\rangle=-r^\ast$ the right-hand side is
...
[spherical realization transfers under reparametrization]
  ...
  If $\gamma$ realizes the spherical curvature $\mu$
  (\cref{def:realizes_spherical_curvature}) and $\psi:\mathbb R\to\mathbb R$ is $C^1$
  with $\psi'>0$ everywhere, then $\gamma\circ\psi$ realizes $\mu\circ\psi$.
\end{lemma}
...
\leanok
  ...
  $(\gamma\circ\psi)'=\psi'\cdot(\gamma'\circ\psi)$, so the speed scales by $\psi'>0$, the
...
$\tilde\gamma'=\rho(\theta)\,e^{i\theta}$, $\rho>0$, i.e.\ (up to the additive constant
$\tilde\gamma(0)$) exactly a Euclidean reconstruction curve
$\mathrm{reconstruct}\,\rho$ (\cref{def:reconstruct}) for the \emph{trajectory speed}
$\rho(\theta)=q_\kappa(\theta,\tilde\gamma(\theta))$. The disk metric plays no role in
...
[trajectory speed: continuity, periodicity, positivity]
  ...
  $R<1$, $\delta>0$, and let
  $\gamma:[0,2\pi]\to\mathbb C$ be a trajectory of the $(\kappa,R,\delta)$-truncated flow
  (in the hypothesis form of \cref{lem:reconstruction_ode}) that is admissible and
  closed. Write $\tilde\gamma$ for its periodic extension and set
  $\rho(t)=q_\kappa\bigl(t,\tilde\gamma(t)\bigr)$
...
\begin{proof}
  ...
  $\theta\mapsto e^{i\theta}$ give $\|\tilde\gamma(t)\|\le R$ and
  $\kappa(t)-\langle\tilde\gamma(t),ie^{it}\rangle\ge\delta$ (the same reduction as inside
  the proof of \cref{lem:reconstruction_ode}).
  \emph{Continuity.} $\tilde\gamma$ is differentiable (the exposed derivative conjunct of
  \cref{lem:reconstruction_ode}), hence continuous; $\rho$ is the quotient
  $\rho=\tfrac{1+\|\tilde\gamma\|^2}{2(\kappa-\langle\tilde\gamma,ie^{it}\rangle)}$ whose
  denominator is continuous and $\ge2\delta>0$ by extended admissibility, so $\rho$ is
  continuous.
  \emph{Periodicity.} $\tilde\gamma(t+2\pi)=\tilde\gamma(t)$
  (\cref{lem:periodic_extension_periodic}), $\kappa(t+2\pi)=\kappa(t)$, and
  $e^{i(t+2\pi)}=e^{it}$; all three inputs of $\rho$ are $2\pi$-periodic.
  \emph{Positivity.} Numerator $1+\|\tilde\gamma\|^2\ge1>0$, denominator $\ge2\delta>0$.
\end{proof}
...
[the closing trajectory is a translated reconstruction curve]
  ...
  \[
    \tilde\gamma(t)\;=\;\tilde\gamma(0)+\mathrm{reconstruct}\,\rho\,(t)
...
\begin{proof}
  ...
  \cref{lem:reconstruction_ode} (which is $q_\kappa(t,\tilde\gamma(t))e^{it}
  ...
  $\tilde\gamma(0)$ since $\mathrm{reconstruct}\,\rho\,(0)=\int_0^0=0$.
\end{proof}
...
[simplicity]
  ...
  closed trajectory $\gamma$ of the truncated flow on
  $[0,2\pi]$), the periodic extension $\tilde\gamma$ is a \emph{simple} closed curve:
  distinct parameters in $[0,2\pi)$ give distinct disk points. (Confinement
  $\|\tilde\gamma\|\le R<1$ already places the curve in the open disk, so this is
...
\begin{proof}
  ...
  =\tilde\gamma(2\pi)-\tilde\gamma(0)=0$ by \cref{lem:spherical_trajectory_eq_reconstruct} and
  $2\pi$-periodicity of $\tilde\gamma$. Hence
  ...
  \cref{lem:is_simple_closed_const_add} (translation by $\tilde\gamma(0)$) transfer
  simplicity to $\tilde\gamma$.
\end{proof}
