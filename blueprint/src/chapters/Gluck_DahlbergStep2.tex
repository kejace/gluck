...
\gluckfigure{euclidean-dahlberg-reconstructions}
  {Three mixed-sign curvature functions satisfying the Euclidean Dahlberg
   pattern and their unit-speed reconstructions. The numbered markers match two
   positive local maxima and two local minima to the corresponding curve
   vertices; other local extrema may remain unmarked. Since every displayed
   curvature is $\pi$-periodic with mean $1$, its tangent angle gains $\pi$ over
   a half-period and the two displacement halves cancel.}
  {Three columns pair sign-changing periodic curvature graphs with smooth closed
   plane curves. In each graph, four selected local extrema are numbered and the
   same numbers appear at the corresponding points on the reconstructed curve.
   Some graphs contain additional unnumbered extrema.}
  {fig:euclidean_dahlberg_reconstructions}
\begin{proof}
