...
% archon:covers Gluck/DahlbergStep2.lean
...
[Closing reparametrisation family --- node-placing arc-length reparam]
  ...
  \lean{Gluck.closingFamily, Gluck.closingDelta1, Gluck.closingDelta2, Gluck.closingDelta3, Gluck.closingDelta4, Gluck.closingDelta5, Gluck.closingLambda, Gluck.closingLambdaRaw, Gluck.closingLambdaFloor, Gluck.closingLen1, Gluck.closingLen2, Gluck.closingLen3, Gluck.closingLen4, Gluck.closingLen5, Gluck.closingS1, Gluck.closingS2, Gluck.closingS3, Gluck.closingS4, Gluck.closingMid1, Gluck.closingMid2, Gluck.closingMid3, Gluck.closingMid4, Gluck.closingMid5, Gluck.closingRamp, Gluck.closingBase, Gluck.closingHeight, Gluck.closingDensity}
  ...
  (and of $k_{a,b}=a(1-f)+bf$, \cref{lem:clean_bicircle_eq_step}). Put
  \[
...
[The calibration scalar $\lambda(z)$ is positive and continuous]
  ...
  \lean{Gluck.closingLambda_pos, Gluck.continuous_closingLambda, Gluck.closingLambda_ne, Gluck.closingLambda_eq_raw, Gluck.continuous_closingLambdaRaw, Gluck.closingLambda_le}
...
[Arc-length interval bounds and total]
  ...
  \lean{Gluck.closingLen_bounds, Gluck.closingLen_sum, Gluck.closingLen_bound_aux}
...
[Basic interface of the node-placing family]
  ...
  \lean{Gluck.closingFamily_zero, Gluck.closingFamily_add_two_pi, Gluck.closingFamily_two_pi, Gluck.strictMono_closingFamily, Gluck.continuous_uncurry_closingFamily, Gluck.hasDerivAt_closingFamily, Gluck.closingDensity_pos, Gluck.closingFamily_slope_bounds, Gluck.closingRamp_pos, Gluck.closingDensity_periodic, Gluck.continuous_closingFamily, Gluck.continuous_closingBase, Gluck.continuous_closingDensity_s, Gluck.continuous_closingLen1, Gluck.continuous_closingLen2, Gluck.continuous_closingLen3, Gluck.continuous_closingLen4, Gluck.continuous_closingLen5, Gluck.continuous_closingMid1, Gluck.continuous_closingMid2, Gluck.continuous_closingMid3, Gluck.continuous_closingMid4, Gluck.continuous_closingMid5, Gluck.continuous_heightAux, Gluck.continuous_lenAux, Gluck.continuous_uncurry_closingDensity, Gluck.continuous_uncurry_term, Gluck.closingDensity_period_integral, Gluck.clampTent_period_integral, Gluck.comp_closingFamily_periodic}
  \uses{def:closing_family, lem:closing_lambda_pos}
...
[Node-mapping of the closing family onto the canonical clean arcs]
  ...
  \lean{Gluck.closingFamily_node, Gluck.closingDensity_arcs, Gluck.closingDensity_integral_split, Gluck.closingHeight_mul, Gluck.half_le_arccos_cos_wide, Gluck.clampTent_integral_eq_zero_wide}
  \uses{def:closing_family, lem:closing_family_props, lem:closing_lambda_pos, lem:closing_len_bounds, lem:clean_bicircle_eq_step}
...
\end{lemma}
\begin{proof}
  \uses{def:closing_family, lem:closing_family_props, lem:clean_bicircle_eq_step}
  ...
  $(s_{j-1},s_j)$ then follows from \cref{lem:clean_bicircle_eq_step}.
\end{proof}
...
[Node-landing: cumulative normalised arc-length angle hits $\mathrm{configSpace}$]
  ...
  \lean{Gluck.cleanArcLength_node_values}
  \uses{def:closing_family, def:arclength_norm, lem:clean_bicircle_eq_step, lem:clean_prefactor, lem:closing_family_nodes}
...
\end{lemma}
\begin{proof}
  \uses{lem:clean_bicircle_eq_step, lem:clean_prefactor, lem:closing_family_nodes}
  \leanok
  By \cref{lem:clean_bicircle_eq_step}, $k_{a,b}$ is a.e.\ the step function with
...
[Normalised arc-length curvature weight]
  ...
  \lean{Gluck.arcLengthNorm, Gluck.dahlbergAngle_arcLengthNorm}
...
[Continuity of the error maps]
  ...
  \lean{Gluck.continuous_arcLengthErrorMap}
...
[Clean total curvature and the positive prefactor $c(z)=1/\lambda(z)$]
  ...
  \lean{Gluck.cleanTotalCurvature_eq, Gluck.cleanPrefactor_pos, Gluck.cleanBicircle_arcs, Gluck.clean_arc_ae, Gluck.clean_arc_int}
  \uses{def:closing_family, def:clean_bicircle, lem:clean_bicircle_eq_step, lem:closing_lambda_pos, lem:closing_len_bounds, lem:closing_family_nodes}
...
\end{lemma}
\begin{proof}
  \uses{lem:clean_bicircle_eq_step, lem:closing_lambda_pos, lem:closing_family_nodes}
  By \cref{lem:clean_bicircle_eq_step}, $k_{a,b}(g_z\,\cdot)$ is a.e.\ the step
...
[Arc-length arc integral on a constant-curvature arc]
  ...
  \lean{Gluck.arcLengthArcIntegral}
...
[Arc-length closure of the clean bicircle $=$ positive multiple of \texttt{errorMap}]
  ...
  \lean{Gluck.arcLengthError_clean_eq_errorMap}
  \uses{def:arclength_error_map, lem:clean_bicircle_eq_step, def:error_map, lem:error_vector_of_bicircle, lem:clean_prefactor, lem:arclength_arc_integral, lem:clean_arclength_nodes}
...
\end{lemma}
\begin{proof}
  \uses{lem:clean_bicircle_eq_step, def:error_map, lem:error_vector_of_bicircle, lem:clean_prefactor, lem:arclength_arc_integral, lem:clean_arclength_nodes}
  \leanok
...
[Bundled preliminary-diffeomorphism data]
  ...
  \lean{Gluck.PreliminaryDiffeoData}
...
[Total curvature stays nonzero]
  ...
  \lean{Gluck.totalCurvature_ne_zero, Gluck.closingFamily_changeOfVar, Gluck.closingFamily_comp_L1, Gluck.measurable_cleanBicircle, Gluck.intervalIntegrable_cleanBicircle_comp}
...
[Angle estimate $|\alpha-\alpha^*|\le C\varepsilon$]
  ...
  \lean{Gluck.angle_dist_le, Gluck.dahlbergAngle_arcLengthNorm_add_two_pi, Gluck.intervalIntegrable_expI_arcLengthNorm, Gluck.angle_dist_arith}
  \uses{def:arclength_error_map, def:preliminary_diffeo_data, lem:total_curvature_ne_zero, lem:clean_prefactor}
...
[Uniform limit $F^*(\cdot,\varepsilon)\to F$]
  ...
  \lean{Gluck.arcLengthErrorMap_tendsto, Gluck.norm_expI_sub_le}
...
[Clean error map: nonzero on the boundary with nonzero winding]
  ...
  \lean{Gluck.cleanError_winds_boundary}
...
[Existence of a closing parameter]
  ...
  \lean{Gluck.exists_closingParam}
...
[Clean chord integrals are bounded away from $0$ on inclination-span-$\le\pi$ arcs]
  ...
  \uses{def:closing_family, lem:closing_lambda_pos, lem:closing_len_bounds, lem:clean_bicircle_eq_step}
...
\begin{proof}
  ...
  $k_{a,b}\in[a,b]$ a.e.\ (\cref{lem:clean_bicircle_eq_step}), there are constants
...
[Simplicity of the perturbed curve (condition (1.3))]
  ...
  \lean{Gluck.simplicity_transport}
  \uses{def:preliminary_diffeo_data, lem:clean_chord_margin, lem:angle_dist_le, thm:exists_closing_param}
...
% Archon-original: a self-contained C¹ circle-inverse construction local to
% DahlbergStep2.lean. The positive-case `exists_C1_circle_inverse` lives PRIVATE in
% FourVertex.lean, which DahlbergStep2.lean does NOT import; rather than a cross-file
% refactor during final assembly, the same IFT+IVT construction is rebuilt here.
\begin{lemma}

\leanok
[$C^1$ circle inverse of an orientation-preserving quasiperiodic map]
  \label{lem:dahlberg_c1_circle_inverse}
  \lean{Gluck.exists_C1_inverse_circle, Gluck.intShift_qp}
  Let $h:\mathbb{R}\to\mathbb{R}$ have a continuous strictly positive derivative
  $v$ ($\forall\theta,\ \mathrm{HasDerivAt}\,h\,(v\theta)\,\theta$, $v>0$, $v$
  continuous) and be quasiperiodic, $h(\theta+2\pi)=h(\theta)+2\pi$. Then there is
  $H:\mathbb{R}\to\mathbb{R}$ with: $H$ continuous and strictly monotone; $h(H t)=t$
  and $H(h t)=t$ for all $t$; $H(t+2\pi)=H(t)+2\pi$; and
  $\mathrm{HasDerivAt}\,H\,(1/v(H t))\,t$ for all $t$ (so $H$ is $C^1$ with positive
  derivative). This is the construction of the positive case
  (\cref{thm:gluck_converse}'s reparam-back), rebuilt in this file.
\end{lemma}
\begin{proof}

\leanok
  $h$ is strictly increasing (positive derivative) and continuous (differentiable);
  the quasiperiodicity gives $h(n\cdot2\pi)=h(0)+n\cdot2\pi$, so $h$ is unbounded
  above and below, hence surjective (IVT). A continuous strictly monotone bijection
  has a continuous strictly monotone two-sided inverse $H$. Quasiperiodicity of $H$
  follows from that of $h$ by applying $H$ to $h(H t+2\pi)=h(Ht)+2\pi=t+2\pi$. The
  derivative of $H$ at $t$ is $1/v(Ht)$ by the inverse-function rule
  (\texttt{HasDerivAt.of\_local\_left\_inverse}, with $v(Ht)\neq0$); continuity of
  $t\mapsto1/v(Ht)$ makes $H$ a $C^1$ map. (Identical to the positive-case
  \texttt{exists\_C1\_circle\_inverse}.)
\end{proof}
...
[Dahlberg converse to the four vertex theorem (Theorem 1.1)]
  ...
  \lean{Gluck.dahlbergConverse}
  \uses{def:mixed_sign_four_vertex, thm:exists_preliminary_diffeo, def:preliminary_diffeo_data, thm:exists_closing_param, lem:total_curvature_ne_zero, lem:simplicity_transport, lem:dahlberg_c1_circle_inverse, lem:closing_family_props, thm:realizes_of_non_normalised, def:non_normalised_curvature}
...
\end{theorem}
\begin{proof}
  \uses{thm:exists_preliminary_diffeo, thm:exists_closing_param, lem:simplicity_transport, lem:total_curvature_ne_zero, lem:dahlberg_c1_circle_inverse, lem:closing_family_props, thm:realizes_of_non_normalised}
  \leanok
  ...
  \cref{lem:dahlberg_c1_circle_inverse} applied to $\varphi$ (with derivative
