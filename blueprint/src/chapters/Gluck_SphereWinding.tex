...
[invariant admissible domain — perturbative margin transport]
  ...
  $0\le R$, $0<\delta$, and let $\gamma^\ast:[0,2\pi]\to\mathbb C$ be a trajectory of the
  $\kappa^\ast$-truncated flow that is \emph{admissible with margin} $\mu>0$:
  \[
    \sup_\theta\|\gamma^\ast(\theta)\|\le R-\mu,
    \qquad
    \sup_\theta\,\langle \gamma^\ast(\theta),\,i e^{i\theta}\rangle\le\kappa_0-\delta-\mu .
  \]
  ...
  $M=\tfrac{1+R^2}{2\delta^2}$. Then every trajectory $\gamma$ of the $\kappa$-truncated flow
  with
  \[
    e^{2\pi L}\Bigl(\|\gamma(0)-\gamma^\ast(0)\|+M\!\int_0^{2\pi}\!|\kappa-\kappa^\ast|\Bigr)\le\mu
  \]
  is admissible: $\|\gamma(\theta)\|\le R$ and
  $\kappa(\theta)-\langle \gamma(\theta),i e^{i\theta}\rangle\ge\delta$ for all
  $\theta\in[0,2\pi]$.
\end{lemma}
...
\leanok
  ...
  of calculus $\gamma(\theta)-\gamma^\ast(\theta)=\gamma(0)-\gamma^\ast(0)+\int_0^\theta\bigl(F_\kappa(s,
  \gamma(s))-F_{\kappa^\ast}(s,\gamma^\ast(s))\bigr)ds$; taking norms and using the field
  sensitivity, $d(\theta)=\|\gamma(\theta)-\gamma^\ast(\theta)\|$ satisfies
  ...
  $\|\gamma(\theta)\|\le\|\gamma^\ast(\theta)\|+\mu\le R$, and since
  $|\langle \gamma-\gamma^\ast,\,i e^{i\theta}\rangle|\le d(\theta)\le\mu$,
  \[
    \kappa(\theta)-\langle \gamma,i e^{i\theta}\rangle
    \ge\kappa_0-\langle \gamma^\ast,i e^{i\theta}\rangle-\mu\ge\delta .
  \]
  ...
  arcs give $\langle \gamma^\ast,ie^{i\theta}\rangle=\langle w_j,ie^{i\theta}\rangle-r_j
...
[periodic extension of a fundamental-window function]
  ...
  For $\gamma:\mathbb R\to\mathbb C$, the \emph{$2\pi$-periodic extension} is
  $\tilde\gamma(t)=\gamma\bigl(t-2\pi\lfloor t/(2\pi)\rfloor\bigr)$: every $t$ is reduced by an
  integer multiple of $2\pi$ into the fundamental window $[0,2\pi)$ and $\gamma$ is evaluated
...
[the periodic extension is periodic]
  ...
  For every $\gamma$, the extension $\tilde\gamma$ of \cref{def:periodic_extension} is
  $2\pi$-periodic: $\tilde\gamma(t+2\pi)=\tilde\gamma(t)$ for all $t$.
\end{lemma}
...
[truncation removal: admissible closed trajectories realize $\kappa$]
  ...
  $\gamma:[0,2\pi]\to\mathbb C$ be a trajectory of the truncated flow that is admissible
  ($\|\gamma(\theta)\|\le R$ and $\kappa(\theta)-\langle \gamma(\theta),ie^{i\theta}\rangle\ge\delta$
  for all $\theta$) and closed ($\gamma(2\pi)=\gamma(0)$). Then the $2\pi$-periodic extension
  $\tilde\gamma$ of $\gamma$ (\cref{def:periodic_extension}) agrees with $\gamma$ on $[0,2\pi]$, is a
  ...
  disk, and solves the \emph{true} reconstruction ODE $\gamma'=q_\kappa(\theta,\gamma)e^{i\theta}$.
  ...
  for $\tilde\gamma$, \emph{plus a fourth conjunct exposing the true-ODE derivative
  globally}: $\forall t,\ \tilde\gamma$ has at $t$ the derivative
  $q_\kappa(t,\tilde\gamma(t))\,e^{it}$. The proof already establishes this fact on the way
  to the realization predicate; exposing it is what lets the simplicity chain
  (\cref{lem:spherical_trajectory_eq_reconstruct}) identify $\tilde\gamma$ with a translated
  ...
  through the admissibility margin $\kappa-\langle \gamma,ie^{i\theta}\rangle\ge\delta$,
...
\leanok
  ...
  right-hand side is continuous, so $\gamma$ is $C^1$ there. Closedness plus
  ...
  positivity from admissibility), so $\gamma'\ne0$, the tangent angle is $\varphi(\theta)=
  \theta$ itself, $\|\gamma'\|=q_\kappa$, and the speed relation of
  \cref{def:realizes_spherical_curvature} is exactly the algebraic solve of
  \cref{def:spherical_speed}. Confinement $\|\gamma\|\le R<1$ gives the open-disk condition.
\end{proof}
...
[constant-curvature arcs are explicit circular arcs]
  ...
  \]
  Then $\gamma(\theta)=w-i\,r\,e^{i\theta}$ solves the true reconstruction ODE
  $\gamma'=q_K(\theta,\gamma)e^{i\theta}$ with constant curvature $K$, pointwise at every $\theta$
  where the bracket stays positive ($K-\langle \gamma(\theta),ie^{i\theta}\rangle>0$): at each
  such $\theta$ one has $q_K(\theta,\gamma(\theta))=r$ and hence
  $\gamma'(\theta)=q_K(\theta,\gamma(\theta))\,e^{i\theta}$. Along the arc
  $\langle \gamma(\theta),ie^{i\theta}\rangle=\langle w,ie^{i\theta}\rangle-r$. Moreover the
...
\leanok
  Differentiate: $\gamma'(\theta)=r e^{i\theta}$, so the claim is $q_K(\theta,\gamma(\theta))=r$.
  Compute $\langle \gamma,ie^{i\theta}\rangle=\langle w,ie^{i\theta}\rangle-r$ and
  $\|\gamma\|^2=\|w\|^2-2r\langle w,ie^{i\theta}\rangle+r^2$; then
  $1+\|\gamma\|^2=2r(K-\langle \gamma,ie^{i\theta}\rangle)$ reduces to
...
[half-turn equivariance]
  ...
  $\gamma(\cdot)$ solves the truncated ODE on $[0,2\pi]$ then $y(\theta)=-\gamma(\theta+\pi)$
  ...
  trajectory through $z_0$ satisfies the central symmetry $\gamma(\theta+\pi)=-\gamma(\theta)$ for
  $\theta\in[0,\pi]$; in particular it closes at $2\pi$: $\gamma(2\pi)=-\gamma(\pi)=z_0$.
\end{lemma}
...
\leanok
  ...
  $y'(\theta)=-\gamma'(\theta+\pi)=-\hat q\,e^{i(\theta+\pi)}=\hat q(\theta,y(\theta))e^{i\theta}$
  ...
  $y(0)=-\gamma(\pi)=z_0=\gamma(0)$, and $\gamma$ restricted to $[0,\pi]$ solves the same ODE
  (\cref{lem:spherical_flow_spec}), so $y=\gamma$ on $[0,\pi]$ by the two-solution
  uniqueness on $[0,\pi]$ (\cref{lem:truncated_field_solution_unique} with $T=\pi$),
  i.e.\ $\gamma(\theta+\pi)=-\gamma(\theta)$ for $\theta\in[0,\pi]$; at $\theta=\pi$ this gives
  $\gamma(2\pi)=-\gamma(\pi)=z_0$.
\end{proof}
...
[arc concatenation]
  ...
  If the bracket $K-\langle \gamma(\theta),ie^{i\theta}\rangle$ stays positive along the arc
  trajectory $\gamma(\theta)=w-ire^{i\theta}$ for $\theta\in[\theta_0,\theta_0+\Delta_1+
  \Delta_2]$, then
  \[
...
\leanok
  ...
  $q_K(\theta_0+\Delta_1,\,\gamma(\theta_0+\Delta_1))=r=q_K(\theta_0,z)$, so the second arc
...
[explicit arcs solve the truncated ODE]
  ...
  $\gamma(\theta)=w-ire^{i\theta}$ be the arc trajectory of \cref{lem:constant_curvature_arc}.
  If along $\theta\in[t_1,t_2]$ the arc is \emph{admissible with clamps inactive} —
  $\|\gamma(\theta)\|\le R$ and $K-\langle \gamma(\theta),ie^{i\theta}\rangle\ge\delta$ — then $\gamma$
  solves the \emph{truncated} ODE for the constant curvature $K$ on $[t_1,t_2]$ in the
  $\mathrm{HasDerivWithinAt}$ sense:
  $\gamma'(\theta)=\hat q_{K,R,\delta}(\theta,\gamma(\theta))\,e^{i\theta}$ on $[t_1,t_2]$.
\end{lemma}
...
\leanok
  ...
  (\cref{lem:truncated_speed_eq}), and $\gamma'(\theta)=re^{i\theta}
  =q_K(\theta,\gamma(\theta))e^{i\theta}$ by \cref{lem:constant_curvature_arc}.
\end{proof}
...
[single-arc margin transport (shifted interval, constant model level)]
  ...
  (\emph{constant} model level), and let $\gamma,\gamma^\ast:[t_1,t_2]\to\mathbb C$ be continuous
  with, in the $\mathrm{HasDerivWithinAt}$ sense on $[t_1,t_2]$,
  \[
    \gamma'=\hat q_{\kappa,R,\delta}(\theta,\gamma)\,e^{i\theta},\qquad
    \gamma^{\ast\prime}=\hat q_{K,R,\delta}(\theta,\gamma^\ast)\,e^{i\theta},
  \]
  and suppose the model has margins $\mu>0$ on the arc:
  $\|\gamma^\ast(\theta)\|\le R-\mu$ and
  $\langle \gamma^\ast(\theta),ie^{i\theta}\rangle\le\kappa_0-\delta-\mu$ for
  $\theta\in[t_1,t_2]$. If
  \[
    e^{L(t_2-t_1)}\Bigl(\|\gamma(t_1)-\gamma^\ast(t_1)\|
      +M\!\int_{t_1}^{t_2}\!|\kappa(s)-K|\,ds\Bigr)\;\le\;\mu,
  \]
  then for all $\theta\in[t_1,t_2]$:
  \[
    \|\gamma(\theta)-\gamma^\ast(\theta)\|\le
    e^{L(t_2-t_1)}\Bigl(\|\gamma(t_1)-\gamma^\ast(t_1)\|+M\!\int_{t_1}^{t_2}\!|\kappa-K|\Bigr),
    \quad
    \|\gamma(\theta)\|\le R,\quad
    \kappa(\theta)-\langle \gamma(\theta),ie^{i\theta}\rangle\ge\delta .
  \]
\end{lemma}
...
\leanok
  ...
  propagation via $|\langle \gamma-\gamma^\ast,ie^{i\theta}\rangle|\le\|\gamma-\gamma^\ast\|$.
\end{proof}
...
[quarter-arc margin package]
  ...
  $\gamma^\ast(\theta)=w-i\,r\,e^{i\theta}$ through $(t_1,p)$ (with $r=q_K(t_1,p)$,
  $w=p+i\,r\,e^{it_1}$), for every $\theta\in[t_1,t_2]$:
  \[
    \|\gamma^\ast(\theta)\|\le R-\mu,\qquad
    \langle \gamma^\ast(\theta),ie^{i\theta}\rangle\le\kappa_0-\delta-\mu,\qquad
    K-\langle \gamma^\ast(\theta),ie^{i\theta}\rangle\ge\delta .
  \]
...
[one quarter-arc of the step transport]
  ...
  $M=\tfrac{1+R^2}{2\delta^2}$. Let $\gamma$ solve the $\kappa$-truncated ODE on
  ...
  \[
    e^{L(t_2-t_1)}\Bigl(\|\gamma(t_1)-p\|+M\!\int_{t_1}^{t_2}\!|\kappa-K|\Bigr)\le\mu .
  \]
  Then $\gamma$ is admissible on $[t_1,t_2]$ ($\|\gamma\|\le R$, bracket $\ge\delta$) and
  \[
    \bigl\|\gamma(t_2)-A_{K,t_1,t_2-t_1}(p)\bigr\|
    \le e^{L(t_2-t_1)}\Bigl(\|\gamma(t_1)-p\|+M\!\int_{t_1}^{t_2}\!|\kappa-K|\Bigr)
  \]
...
[four-arc chained transport against the step model]
  ...
  (3\pi/2)$. Let $\gamma$ be a trajectory of the $\kappa$-truncated flow on $[0,2\pi]$ with
  $\gamma(0)=z_0$, and let the \emph{concatenated step model} from $z_0$ be the four
  ...
  \]
  Then $\gamma$ is admissible on $[0,2\pi]$ ($\|\gamma\|\le R$, bracket $\ge\delta$), and the
  endpoint satisfies
  \[
    \bigl\|\,\bigl(\gamma(2\pi)-z_0\bigr)-E^\ast_{a,b}(z_0)\,\bigr\|
...
\begin{proof}
  ...
  bounds: with $d_j=\|\gamma(\theta_j)-p_j\|$ and $I_j=M\int_{\theta_j}^{
...
\begin{proof}
  ...
  $\|\gamma^\ast\|\equiv r^\ast<1$ and $\langle \gamma^\ast(\theta),ie^{i\theta}\rangle\equiv
  ...
  Along each arc,
  $\langle \gamma^\ast(\theta),ie^{i\theta}\rangle=\langle w_j,ie^{i\theta}\rangle-r_j$
