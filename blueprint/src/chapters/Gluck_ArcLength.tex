...
% archon:covers Gluck/ArcLength.lean
...
The mixed-sign construction (Phases D-B, D-C) instead produces, after a
reparametrisation, a function $\kappa\circ\varphi$ that is a \emph{non-normalised}
curvature function: a rescaling and reparametrisation recover the theorem. This
section records that reduction, which is what D-B/D-C must establish.
...
with $\kappa\circ\varphi$ non-normalised. Phase D-B (planned chapter)
constructs the preliminary diffeomorphism $\eta$ giving $\kappa\circ\eta=a(1-f)+bf
+e$ with $\int_0^{2\pi}|e|<C\varepsilon$; Phase D-C (planned) finds the closing
fractional-transformation parameter via the in-tree winding/degree core and the
$|\alpha-\alpha^*|\le C\varepsilon$ transport of conditions (1.2)/(1.3).
