...
%     RealizesHyperbolicCurvature, HyperbolicFourVertex, hyperbolicConverse_pos
%     (the ε = −1 instance of SpaceForm.spaceFormConverse_pos);
%   Gluck/Hyperbolic/Mixed.lean — mixed-sign wrappers:
%     MixedHyperbolicFourVertex, hyperbolicMixedConverse (up to reparam),
%     hyperbolicMixedConverse_exact (re-exports of the Hyperbolic.* results);
%   Gluck/Hyperbolic/Exact.lean — degree-one reparam removal:
%     injOn_Ico_shift_of_periodic, realizes_of_reparam_degree_one,
%     Hyperbolic.hyperbolicMixedConverse_exact (closes the AL-6 gap).
...
\item \emph{Positive (escape-velocity) converse},
  ...
  so $\kappa>1$ is the natural positivity hypothesis. This is the $\varepsilon=-1$
  instance of the $\varepsilon$-generic space-form positive converse; nothing
...
\item \emph{Exact-profile converse} — the headline of the chapter —
  ...
  $\kappa$ \emph{exactly},
  \[
    \exists\,z,\quad \texttt{IsSimpleClosed }z\ \wedge\
    \texttt{Realizes }(-1)\ z\ \kappa,
  \]
  with \emph{no} reparametrization. The up-to-reparam form
  \cref{thm:hyperbolic_mixed_converse} is kept as a weakening corollary.
\end{itemize}

\gluckfigure{hyperbolic-escape}
  {Centered hyperbolic circles approaching the escape threshold. For
   $c>1$, the disk radius is $r(c)=c-\sqrt{c^2-1}$; as $c\downarrow1$ the
   circle approaches the ideal boundary while the admissibility bracket
   $\sqrt{c^2-1}$ collapses to zero.}
  {Three nested circles in the Poincare disk. Circles with curvature two, six
   fifths, and fifty-one fiftieths grow toward the dashed ideal boundary as the
   curvature approaches one from above.}
  {fig:hyperbolic_escape_threshold}

\paragraph{Exact versus up-to-reparam.}
\gluckfigure{hyperbolic-reconstructions}
  {Three Poincaré-disk representatives on $H^2$: two curvature profiles stay
   strictly above the escape threshold $\kappa_H=1$, while the third enters the
   mixed-sign regime. Four selected extrema are matched to their curve points;
   additional vertices are intentionally left unmarked.}
  {Three columns pair hyperbolic geodesic-curvature graphs with smooth closed
   curves in the Poincaré disk. The first two graphs remain above a marked level
   one, and the third crosses zero. Matching numbered markers connect four
   selected extrema with points on each curve.}
  {fig:hyperbolic_reconstructions}
...
global $C^1$ inverse $\Psi^{-1}$ exists, and $w=z\circ\Psi^{-1}$ is a simple
...
The $\varepsilon$-generic packaging of these results lives in
...
[hyperbolic realization]
  ...
  A curve $z:\mathbb R\to\mathbb C$ \emph{realizes the hyperbolic curvature
  function} $\kappa$ if $\texttt{SpaceForm.Realizes}\,(-1)\,z\,\kappa$: against
  the tangent-angle gauge $\varphi$, the defining speed relation is
  \[
    \frac{1-\lVert z\rVert^2}{2}\,\varphi'
      \;=\;\bigl(\kappa+\langle z,\,ie^{i\varphi}\rangle_{\mathbb R}\bigr)
      \,\lVert z'\rVert
  \]
  (note the $+$ inner-product term, versus the $-$ of the spherical
  \cref{def:realizes_spherical_curvature}, and the metric factor
  $1-\lVert z\rVert^2$). Definitionally the $\varepsilon=-1$ instance of the
  $\varepsilon$-generic predicate; the accompanying lemma
...
[hyperbolic four-vertex hypothesis]
  ...
  The $\varepsilon=-1$ instance of \texttt{SpaceForm.SpaceFormFourVertex}: a
  ...
  $\varepsilon=-1$ is negative, the conditional escape guard of the generic
...
[positive hyperbolic converse]
  ...
  \lean{Gluck.hyperbolicConverse_pos}
  ...
  \[
    ...
    \texttt{RealizesHyperbolicCurvature }z\,\kappa .
  \]
...
\begin{proof}
  ...
  \leanok
  One line: the $\varepsilon=-1$ case of the space-form positive converse,
  $\texttt{SpaceForm.spaceFormConverse\_pos}\ (\texttt{Or.inr rfl})$. All the
  work — flow, winding, reconstruction, simplicity — happens
  $\varepsilon$-generically in \cref{chap:spaceform_converse}.
\end{proof}
...
\begin{theorem}[Hyperbolic mixed converse, up to reparametrization]
  ...
  the Poincar\'e disk:
  \[
    \exists\,z\,\Psi,\quad C^1\,\Psi\ \wedge\ 0<\Psi'\ \wedge\
    \texttt{IsSimpleClosed }z\ \wedge\
    \texttt{RealizesHyperbolicCurvature }z\,(\kappa\circ\Psi).
  \]
...
\begin{proof}
  ...
  (\texttt{Hyperbolic.hyperbolicMixedConverse},
...
[reparam removal for degree-one reparametrizations]
  ...
  Let $\varepsilon\in\mathbb R$ and suppose a simple closed curve $z$ realizes
  $\kappa\circ\Psi$ at sign $\varepsilon$, where $\Psi$ is $C^1$,
  ...
  by the simple closed curve $w=z\circ\Psi^{-1}$:
  \[
    \exists\,w,\quad \texttt{IsSimpleClosed }w\ \wedge\
    \texttt{Realizes }\varepsilon\ w\ \kappa .
  \]
  Stated $\varepsilon$-generically: nothing in the removal is hyperbolic.
\end{theorem}
...
\begin{proof}
  ...
  \emph{$w=z\circ\Phi$ is simple closed.} Closedness: $w(t+2\pi)
  =z(\Phi(t)+2\pi)=z(\Phi(t))=w(t)$ by the degree-one identity for $\Phi$ and
  $2\pi$-periodicity of $z$. Injectivity on $[0,2\pi)$: $\Phi$ maps
  $[0,2\pi)$ strictly monotonically into the \emph{shifted} fundamental
  interval $[\Phi(0),\Phi(0)+2\pi)$, on which $z$ is injective by
  ...
  gives $\texttt{Realizes }\varepsilon\,(z\circ\Phi)\,((\kappa\circ\Psi)\circ\Phi)$,
...
[exact-profile hyperbolic mixed converse]
  ...
  \lean{Gluck.hyperbolicMixedConverse_exact}
  \uses{def:mixed_hyperbolic_four_vertex, def:realizes_hyperbolic_curvature,
...
\begin{proof}
  ...
  (\cref{chap:hyperbolic_family}) supplies a simple closed $z$ realizing
  ...
  $w=z\circ\Psi^{-1}$. The user-facing \texttt{Gluck.hyperbolicMixedConverse\_exact}
