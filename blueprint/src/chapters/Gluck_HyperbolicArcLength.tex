...
\emph{convex-only} for $K=-1$: its trajectories have monotone tangent
angle and force the admissibility bracket
$\kappa-K\langle z,ie^{i\theta}\rangle>0$, so negative geodesic
...
\[
  \gamma'(\sigma) = e^{i\varphi(\sigma)}, \qquad
  \varphi'(\sigma) =
  \frac{2\bigl(\kappa(\sigma) + \langle \gamma(\sigma),
    i e^{i\varphi(\sigma)}\rangle\bigr)}{1 - \|\gamma(\sigma)\|^{2}},
\]
a coupled system in $W=(\gamma,\varphi)\in\{\|z\|<1\}\times\mathbb R$. The
...
$\|\gamma\|<1$ is not automatic (unit Euclidean speed can reach the boundary) and is
...
\begin{lemma}\leanok
[a confined solution realizes $\kappa$ at $K=-1$]
  \label{lem:arc_solution_realizes}
  \lean{Gluck.Hyperbolic.arcSolution_realizes}
  \uses{def:arc_angle_speed, def:spaceform_realizes}
  If $(\gamma,\varphi)$ solves the \emph{true} (unclamped) system and stays confined
  ($\|\gamma(\sigma)\|<1$ for all $\sigma$), then
  $\mathrm{Realizes}\,(-1)\,\gamma\,\kappa$ with tangent angle $\varphi$. No new
...
[constant-curvature arc model]
  ...
  $\gamma(\sigma)=z_0-r\,ie^{i\varphi_0}(e^{i\sigma/r}-1)$,
...
[H\texorpdfstring{$^2$}{2} arc-length curvature function]
  ...
  $(\gamma,\varphi)$ globally solving the H\textsuperscript{2} arc-length system,
  confined ($\|\gamma\|<1$), closing ($\gamma(L)=\gamma(0)$) with total turning $2\pi$
  ($\varphi(L)=\varphi(0)+2\pi$), $L$-periodic in $\gamma$, and injective on
...
[floor-glued periodic extension of a window solution]
  ...
  is a global solution of the true system, $L$-periodic in $\gamma$, confined,
...
\begin{proof}
  ...
  (\texttt{injOn\_arcCurve}): the FTC bridge $\gamma(b)-\gamma(a)=\int_a^b e^{i\varphi}$
...
[the converse at an explicit window, with the shift law]
  ...
  Given a confined, $L$-periodic-in-$\gamma$, simple global solution $(\gamma,\varphi)$
  ...
  produces $Z=\gamma\circ\chi$ with: $\chi$ is $C^1$ with $\chi'>0$, the
...
\begin{proof}
  ...
  \leanok
  \cref{lem:arc_solution_realizes} gives $\mathrm{Realizes}\,(-1)\,\gamma\,\kappa$;
  ...
  onto $[0,L)$, transporting $L$-periodicity of $\gamma$ to $2\pi$-periodicity of
...
[the H\texorpdfstring{$^2$}{2} arc-length converse, up to reparametrization]
  ...
  function, there are a simple closed curve $\gamma$ and an orientation-preserving
  $C^1$ reparametrization $\psi$ with
  \[
    \texttt{IsSimpleClosed}\,\gamma \;\wedge\;
    \mathrm{Realizes}\,(-1)\,\gamma\,(\kappa\circ\psi).
  \]
...
[realization up to reparametrization]
  ...
  $\texttt{IsSimpleClosed}\,\gamma\wedge\mathrm{Realizes}\,(-1)\,\gamma\,(\kappa\circ\Psi)$.
