...
\newpage
\illustrationpage{discrete-gluck-menger-reconstructions}
  {Three discrete Euclidean Gluck reconstructions}
  {Each stem is the signed Menger curvature computed from three consecutive
   vertices of the polygon directly below it. The four numbered samples
   alternate high--low--high--low across a common window level; all samples are
   positive, and additional unmarked extrema are allowed.}

\newpage
\illustrationpage{discrete-menger-bridge}
  {The Euclidean tangent--chord/Menger bridge}
  {At vertex $i$, the signed Menger curvature is the reciprocal circumradius.
   The two adjacent edge lengths determine the turning angle through the
   tangent--chord law. Globally, the flexible lengths must also close the
   position gap and produce total turning $2\pi$.}

\newpage
\illustrationpage{menger-curvature-spaceforms}
  {Menger curvature in the three space forms}
  {Three consecutive vertices determine a unique ambient cycle. Their signed
   Menger curvature is the cycle's signed geodesic curvature. Euclidean circles,
   spherical circles, and all hyperbolic cycle types therefore belong to one
   intrinsic definition.}

\newpage
\illustrationpage{menger-curvature-profiles-spaceforms}
  {From embedded triples to a discrete curvature profile}
  {At every polygon vertex, fit the cycle through the preceding, current, and
   following vertices, then record its signed geodesic curvature as $\kappa_i$.
   Conformal coordinates make the spherical and hyperbolic values explicit;
   the highlighted vertex corresponds to the highlighted profile sample.}
...
\illustrationpage{spaceform-unification}
  {Analogous reconstructions across all three space forms}
  {Three nonconstant periodic four-vertex profiles are paired with their own
   spherical, Euclidean, and hyperbolic reconstructions. Correspondence is
   asserted within each column, not between different ambient metrics.}
