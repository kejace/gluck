...
private lemma reconstruction_extended_admissible {κ : ℝ → ℝ} {R δ : ℝ}
...
the *true* reconstruction ODE `z' = q_κ(θ, z)·e^{iθ}`: both clamps are
inactive, so the truncated field equals the gauge speed times the tangent. -/
private lemma reconstruction_hasDerivWithinAt_true {κ : ℝ → ℝ} {R δ : ℝ}
    {z : ℝ → ℂ}
...
private lemma reconstruction_shifted_hasDerivWithinAt {κ : ℝ → ℝ}
    (hκper : Function.Periodic κ (2 * π)) {z : ℝ → ℂ}
...
`HasDerivAt`. -/
private lemma reconstruction_hasDerivAt_seam {κ : ℝ → ℝ} {z : ℝ → ℂ}
...
private lemma reconstruction_hasDerivAt {κ : ℝ → ℝ} {z : ℝ → ℂ}
    (hshifted : ∀ n : ℤ, ∀ u ∈ Set.Icc ((n : ℝ) * (2 * π))
        ((n : ℝ) * (2 * π) + 2 * π),
      HasDerivWithinAt (fun t : ℝ => z (t - (n : ℝ) * (2 * π)))
        (sphericalSpeed κ u (z (u - (n : ℝ) * (2 * π))) •
...
positive: the numerator `1 + ‖z‖²` is positive and the denominator
`2(κ − ⟪z, i e^{iθ}⟫)` is bounded below by `2δ > 0`. -/
private lemma reconstruction_speed_pos {κ : ℝ → ℝ} {R δ : ℝ} (hδ : 0 < δ)
    {z : ℝ → ℂ}
    (hadmZ : ∀ t : ℝ, ‖periodicExtension z t‖ ≤ R ∧
      δ ≤ κ t - ⟪periodicExtension z t,
        Complex.I * Complex.exp ((t : ℂ) * Complex.I)⟫_ℝ)
    (t : ℝ) : 0 < sphericalSpeed κ t (periodicExtension z t) := by
  have h := (hadmZ t).2
  unfold sphericalSpeed
  exact div_pos (by positivity) (by linarith)

/-- **`C¹` regularity of the extension.** The derivative `q_κ(t, z)·e^{it}` is
continuous: the speed is a quotient of continuous functions with nonvanishing
denominator, and `e^{it}` is continuous. -/
private lemma reconstruction_contDiff {κ : ℝ → ℝ} {R δ : ℝ} (hκc : Continuous κ)
    (hδ : 0 < δ) {z : ℝ → ℂ}
    (hadmZ : ∀ t : ℝ, ‖periodicExtension z t‖ ≤ R ∧
      δ ≤ κ t - ⟪periodicExtension z t,
        Complex.I * Complex.exp ((t : ℂ) * Complex.I)⟫_ℝ)
...
/-- **The extension realizes `κ`.** Assembles `RealizesSphericalCurvature` in
the gauge `φ = id`: `C¹` regularity, regularity of the derivative, confinement
to the open disk, the tangent-angle equation and the gauge-speed relation. -/
private lemma reconstruction_realizesSphericalCurvature {κ : ℝ → ℝ} {R δ : ℝ}
    (hκc : Continuous κ) (hR1 : R < 1) (hδ : 0 < δ) {z : ℝ → ℂ}
    (hadmZ : ∀ t : ℝ, ‖periodicExtension z t‖ ≤ R ∧
      δ ≤ κ t - ⟪periodicExtension z t,
        Complex.I * Complex.exp ((t : ℂ) * Complex.I)⟫_ℝ)
    (hspeed_pos : ∀ t : ℝ, 0 < sphericalSpeed κ t (periodicExtension z t))
    (hZderiv : ∀ t, HasDerivAt (periodicExtension z)
      (sphericalSpeed κ t (periodicExtension z t) •
        Complex.exp ((t : ℂ) * Complex.I)) t) :
...
lemma reconstruction_ode {κ : ℝ → ℝ} {R δ : ℝ} (hκc : Continuous κ)
  ...
  have hspeed_pos := reconstruction_speed_pos hδ hadmZ
  exact ⟨reconstruction_eqOn hZeq, periodicExtension_periodic z,
    reconstruction_realizesSphericalCurvature hκc hR1 hδ hadmZ hspeed_pos hZderiv,
    hZderiv⟩
