...
Authors: kejace
-/
import Gluck.Hyperbolic.MixedSign.Closing
...
private lemma chord_ne_zero_of_lift {z : ℝ → ℂ} {φ : ℝ → ℝ} {θ : ℝ → ℝ} {L : ℝ} (hL : 0 < L)
    (hzd : ∀ σ ∈ Set.Icc (0 : ℝ) L,
...
private lemma lift_field_identity {z e : ℂ} (hz : z ≠ 0) :
...
private lemma lift_identity_of_deriv {z : ℝ → ℂ} {φ θ : ℝ → ℝ} {L : ℝ} (hL0 : 0 ≤ L)
    (hzd : ∀ σ ∈ Set.Icc (0 : ℝ) L,
      HasDerivWithinAt z (Complex.exp ((φ σ : ℂ) * Complex.I)) (Set.Icc 0 L) σ)
...
private lemma arcModelConst_inner_scalar {K : ℝ} {z₀ : ℂ} {φ₀ : ℝ}
...
private lemma neg_arc1_inner {h σ : ℝ}
...
private lemma neg_arc1_inner_ub {h L σ : ℝ} (h1 : (1 : ℝ) / 10 ≤ h) (h2 : h ≤ 3 / 20)
...
private lemma mixed_radial_lift {δ h L : ℝ}
    (hδ : 0 < δ) (hh1 : (1 : ℝ) / 10 ≤ h) (hh2 : h ≤ 3 / 20)
    (hL1 : (157 : ℝ) / 50 ≤ L) (hL2 : L ≤ 161 / 50)
    (hδC : negRobustConst * δ ≤ 1 / 200)
...
private lemma mixed_chord_ne_zero {δ h L : ℝ}
    (hδ : 0 < δ) (hh1 : (1 : ℝ) / 10 ≤ h) (hh2 : h ≤ 3 / 20)
    (hL1 : (157 : ℝ) / 50 ≤ L) (hL2 : L ≤ 161 / 50)
    (hδC : negRobustConst * δ ≤ 1 / 200)
    (him : (arcFlow (arcRampProfile (-3 / 10) 2 L δ) (4 / 5) L 2 4
...
proved as `hyperbolicMixedConverse` in `Gluck/Hyperbolic/Family.lean`.
...
/-! ## Wrapper (planned `Gluck/Hyperbolic/Mixed.lean`, mirror `Gluck/Hyperbolic.lean`)
