...
(`ε=0`) it is `c` itself, positive on the flat window `c > 1/2`. So the margin
...
lemma centeredRadius_facts {ε c : ℝ} (hε : ε = 1 ∨ ε = -1 ∨ ε = 0)
  ...
  have hbr := centeredRadius_bracket ε c hε hc
...
private lemma spaceFormSpeed_near_circle {ε c K t₁ h d : ℝ} {p : ℂ}
  ...
  have hεabs : |ε| ≤ 1 := eps_abs_le_one hε
  ...
  have hquadbound : |spaceFormSpeed ε (fun _ => c) t₁ p - rs| ≤ d ^ 2 / B := by
    ...
    rw [hquad, hrw]
    exact le_trans (abs_eps_mul_le hεabs _)
      (quadratic_deviation_bound hBpos (sq_nonneg _) hNnorm hcεβ)
...
lemma stepModel_margins {ε c κ₀ : ℝ} (hε : ε = 1 ∨ ε = -1 ∨ ε = 0)
    (hc : (ε = 1 ∧ 0 < c) ∨ (ε = -1 ∧ 1 < c) ∨ (ε = 0 ∧ 1 / 2 < c))
