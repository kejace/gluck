...
private lemma sf_bnd_T2 {s R ε η h D β U dz : ℝ} (hs0 : 0 < s) (hR0 : 0 < R)
    (hR1 : R < 1) (hη : |η| ≤ h / 2) (hβ2 : (ε * β) ^ 2 ≤ U ^ 2)
  ...
  have hNb : |R * η * (ε * β) ^ 2| ≤ h / 2 * U ^ 2 := by
    rw [abs_mul, abs_mul, abs_of_nonneg hR0.le, abs_of_nonneg (sq_nonneg (ε * β))]
    nlinarith [mul_nonneg (mul_nonneg (show (0:ℝ) ≤ 1 - R by linarith) (abs_nonneg η))
        (sq_nonneg (ε * β)),
      mul_nonneg (show (0:ℝ) ≤ h / 2 - |η| by linarith) (sq_nonneg (ε * β)),
      mul_nonneg (show (0:ℝ) ≤ h / 2 by linarith)
        (show (0:ℝ) ≤ U ^ 2 - (ε * β) ^ 2 by linarith),
      hR0, abs_nonneg η]
...
private lemma sf_bnd_T4 {s ε U η h D dz dz0 : ℝ} (hs0 : 0 < s) (hεabs : |ε| ≤ 1)
  ...
  have hNb : |ε * U ^ 2 * η| ≤ U ^ 2 * (h / 2) := by
    rw [show ε * U ^ 2 * η = ε * (U ^ 2 * η) from by ring]
    refine le_trans (abs_eps_mul_le hεabs _) ?_
...
private lemma sf_bnd_Y1 {s ε IuyG β U Uy Gn D h dz0 : ℝ} (hs0 : 0 < s)
    (hIuyGb : |IuyG| ≤ Uy * Gn) (hβU : |β| ≤ U) (hεabs : |ε| ≤ 1)
  ...
  have hNb : |ε * IuyG * (ε * β)| ≤ Uy * Gn * U := by
    have h2 : |ε * β| ≤ U := le_trans (abs_eps_mul_le hεabs β) hβU
    rw [show ε * IuyG * (ε * β) = ε * (IuyG * (ε * β)) from by ring]
    refine le_trans (abs_eps_mul_le hεabs _) ?_
    rw [abs_mul]
...
private lemma sf_bnd_Y2 {s ε Gn D h dz0 : ℝ} (hs0 : 0 < s) (hεabs : |ε| ≤ 1)
  ...
  have hNb : |ε * Gn ^ 2| ≤ Gn ^ 2 :=
    le_trans (abs_eps_mul_le hεabs _) (abs_of_nonneg (sq_nonneg _)).le
...
private lemma sf_bnd_Y3 {s ε β βy Uy Gn D h dz0 dyy : ℝ} (hs0 : 0 < s)
    (hεabs : |ε| ≤ 1) (hβby : |β - βy| ≤ Gn) (hUy2 : Uy ^ 2 ≤ 4 * D ^ 2)
  ...
  have hNb : |ε * Uy ^ 2 * (ε * β - ε * βy)| ≤ Uy ^ 2 * Gn := by
    have h1 : |ε * β - ε * βy| ≤ Gn := by
      rw [show ε * β - ε * βy = ε * (β - βy) from by ring]
      exact le_trans (abs_eps_mul_le hεabs _) hβby
    rw [show ε * Uy ^ 2 * (ε * β - ε * βy) = ε * (Uy ^ 2 * (ε * β - ε * βy)) from by ring]
    refine le_trans (abs_eps_mul_le hεabs _) ?_
...
private lemma sf_bnd_FR {s R ε η h D β IδV Nud : ℝ} (hs0 : 0 < s) (hR0 : 0 < R)
    (hR1 : R < 1) (hη : |η| ≤ h / 2) (hεabs : |ε| ≤ 1) (hβδ : |β - IδV| ≤ Nud)
  ...
  have hRε : |R * ε| ≤ R := by
    ...
    exact mul_le_of_le_one_right hR0.le hεabs
  have hNb : |R * ε * η * (β - IδV)| ≤ h / 2 * Nud := by
    calc |R * ε * η * (β - IδV)| = |R * ε| * |η| * |β - IδV| := by rw [abs_mul, abs_mul]
      _ ≤ R * (|η| * |β - IδV|) := by
          rw [mul_assoc]
          exact mul_le_mul_of_nonneg_right hRε (by positivity)
      _ ≤ 1 * (h / 2 * Nud) :=
...
private lemma sf_arcSpeed_decomp {ε c h η θ : ℝ} {δ z y G : ℂ} {Ag : ℝ}
  ...
  have hεβabs : |ε * β| ≤ ‖u‖ := le_trans (abs_eps_mul_le hεabs β) hβU
  have hεβuyabs : |ε * βuy| ≤ ‖uy‖ := le_trans (abs_eps_mul_le hεabs βuy) hβuyU
  ...
  have hεβ2 : (ε * β) ^ 2 ≤ ‖u‖ ^ 2 := by
    rw [← sq_abs]; exact pow_le_pow_left₀ (abs_nonneg _) hεβabs 2
  ...
  have hFR : |R*ε/s^2*η*(β - ⟪δ,v⟫_ℝ)| ≤ 400*((1/s+1/s^3)*(h*(‖δ‖^2+h))) := by
    ...
    have hNb : |R*ε*η*(β-⟪δ,v⟫_ℝ)| ≤ h/2 * ‖u-δ‖ := by
      ...
      have hRε : |R * ε| ≤ R := by
        ...
        exact mul_le_of_le_one_right hrs0.le hεabs
      rw [hβδv]
      calc |R * ε * η * ⟪u-δ,v⟫_ℝ| = |R * ε| * |η| * |⟪u-δ,v⟫_ℝ| := by
            rw [abs_mul, abs_mul]
        _ ≤ R * (|η| * |⟪u-δ,v⟫_ℝ|) := by
            rw [mul_assoc]
            exact mul_le_mul_of_nonneg_right hRε (by positivity)
  ...
  have hIS : |(ε*⟪uy,g⟫_ℝ - ε*⟪δ,G⟫_ℝ)/s| ≤ 100*((1/s+1/s^3)*(h*(‖δ‖^2+h))) + ‖δ‖*Ag/s := by
    ...
    rw [show ε*⟪uy,g⟫_ℝ - ε*⟪δ,G⟫_ℝ = ε*(⟪uy,g⟫_ℝ - ⟪δ,G⟫_ℝ) from by ring]
    refine le_trans (abs_eps_mul_le hεabs _) ?_
  ...
  have hT2 := sf_bnd_T2 hs0 hrs0 hrs1 hη hεβ2 hhU2 hh0.le hδ0 (norm_nonneg u) hdzKf
  have hT3 := sf_bnd_T3 hs0 hrs0 hrs1 hη2 hεβabs (by linarith [husmall]) hh0.le hδ0 (norm_nonneg u)
    hdzKf
  have hT4 := sf_bnd_T4 hs0 hεabs hη hhU2 hh0.le hδ0 hdzKf hdz0f
  have hY1 := sf_bnd_Y1 hs0 hIuyGb hβU hεabs hgc hun huyn hDs' hh0.le hδ0
    (norm_nonneg g) (norm_nonneg u) (norm_nonneg uy) hdz0f
  have hY2 := sf_bnd_Y2 (D := ‖δ‖) hs0 hεabs hgc (norm_nonneg g) hh0.le hδ0 hdz0f
  have hY3 := sf_bnd_Y3 hs0 hεabs hβby hUy2 hgc (norm_nonneg g) hh0.le hδ0 hdz0f hdyf
...
private lemma sf_stepError_arc1 {ε c h : ℝ} {δ z₁ W : ℂ} {Q₀ Q₁ r s R κ Bres : ℝ}
  ...
  have hmain₁ : |R / s * -(h / 2) + R * ε / s ^ 2 * -(h / 2) * -δ.re
      + ε * κ * (δ.re + δ.im) / s| ≤ h / s :=
    sf_mainbnd hs0 hrs0 hrs1 hh0.le hεabs hη1 (by rw [abs_neg]; exact hδre)
      (sf_absKap hs0 hκ0 hκs hh0.le hεabs (Gluck.abs_re_add_im_le δ)) hδs
  ...
  have hQ₁κ : |Q₁ - r + κ| ≤ 2 * Bres + 2 * h * ‖δ‖ / s ^ 2 := by
    ...
    have hl1 : |R * ε / s ^ 2 * -(h / 2) * -δ.re| ≤ h * ‖δ‖ / (2 * s ^ 2) :=
      sf_absR2 (x := -δ.re) hs0 hrs0 hrs1 hh0.le hεabs hη1 (by rw [abs_neg]; exact hδre)
    have hl2 : |ε * κ * (δ.re + δ.im) / s| ≤ h * ‖δ‖ / s ^ 2 :=
      sf_absKap hs0 hκ0 hκs hh0.le hεabs (Gluck.abs_re_add_im_le δ)
...
private lemma sf_stepError_arc2 {ε c h : ℝ} {δ z₂ W : ℂ} {Q₀ Q₁ Q₂ r s R κ Bres : ℝ}
  ...
  have hg₂g : |ε * (2 * κ * δ.re) / s| ≤ h * ‖δ‖ / s ^ 2 := by
    ...
    refine sf_absKap hs0 hκ0 hκs hh0.le hεabs ?_
    rw [abs_mul, show |(2:ℝ)| = 2 by norm_num]; linarith [hδre]
  have hmain₂ : |R / s * (h / 2) + R * ε / s ^ 2 * (h / 2) * -δ.im + ε * (2 * κ * δ.re) / s|
      ≤ h / s :=
    sf_mainbnd hs0 hrs0 hrs1 hh0.le hεabs hη0 (by rw [abs_neg]; exact hδim) hg₂g hδs
  ...
  have hQ₂κ : |Q₂ - r - κ| ≤ 2 * Bres + 2 * h * ‖δ‖ / s ^ 2 := by
    ...
    have hl1 : |R * ε / s ^ 2 * (h / 2) * -δ.im| ≤ h * ‖δ‖ / (2 * s ^ 2) :=
      sf_absR2 (x := -δ.im) hs0 hrs0 hrs1 hh0.le hεabs hη0 (by rw [abs_neg]; exact hδim)
...
with `η(ε) = 2·ε·r*/(c² + ε)`. The `ε` factor is mandatory — it descends from
the `ε` on the RHS of `spaceFormSpeed_sub_radius` — so `η` is *positive* for
the sphere (`ε=+1`), *negative* for the hyperbolic plane (`ε=−1`), and *zero*
for the flat plane (`ε=0`, where the four-arc error map is identically zero and
...
`stepError_expansion`.) -/
lemma stepError_expansion {ε c : ℝ} (hε : ε = 1 ∨ ε = -1 ∨ ε = 0)
    (hc : (ε = 1 ∧ 0 < c) ∨ (ε = -1 ∧ 1 < c) ∨ (ε = 0 ∧ 1 / 2 < c)) :
    ∃ ρ₁ h₁ C : ℝ, 0 < ρ₁ ∧ 0 < h₁ ∧ 0 < C ∧
      ∀ h : ℝ, 0 < h → h ≤ h₁ → ∀ z₀ : ℂ,
  ...
  have hεδim : ε * δ.im ≤ ‖δ‖ := le_trans (le_abs_self _)
    (le_trans (abs_eps_mul_le hεabs _) hδim)
  ...
  have hposθ : ∀ φ : ℝ, 0 < c - ε * ⟪W - Complex.I * (r : ℂ)
    ...
    have hεW : ε * ⟪W, Complex.I * Complex.exp ((φ : ℂ) * Complex.I)⟫_ℝ ≤ ‖W‖ :=
      le_trans (le_abs_self _) (le_trans (abs_eps_mul_le hεabs _) h1)
    have hεrR : -(‖δ‖ ^ 2 / s) ≤ ε * (r - R) := by
      have : |ε * (r - R)| ≤ ‖δ‖ ^ 2 / s :=
        le_trans (abs_eps_mul_le hεabs _) hrs_r
  ...
  have hmain₀ : |R / s * (h / 2) + R * ε / s ^ 2 * (h / 2) * δ.im| ≤ h / s := by
    have hz := sf_mainbnd (g := (0 : ℝ)) hs0 hrs0 hrs1 hh0.le hεabs hη0 hδim
  ...
  have hleft₀ : |R * ε / s ^ 2 * (h / 2) * δ.im| ≤ h * ‖δ‖ / (2 * s ^ 2) :=
    sf_absR2 hs0 hrs0 hrs1 hh0.le hεabs hη0 hδim
  ...
  obtain ⟨hres₁, hQ₁r, hQ₁κ⟩ := sf_stepError_arc1 hε hc hh0 hσdec hhdec hsdef hs2 hs0
    hrs0 hrs1 hεabs hV1 hi1 hig1 hWδ hg₁ hrs_r hsp₁ hQ₀r hQ₀κ hκ0 hκs hκdef hδre hη1
  ...
  obtain ⟨hres₂, hQ₂r, hQ₂κ⟩ := sf_stepError_arc2 hε hc hh0 hσdec hhdec hsdef hs2 hs0
    hrs0 hrs1 hεabs hV2 hi2 hig2 hWδ hg₂ hrs_r hsp₂ hQ₀r hQ₁r hQ₀κ hQ₁κ hκ0 hκs hκdef
    hδre hδim hη0 hQ₂def hBresdef hBresnn hBres1 hAgfold hXnn hn1I hnm1I hn2I hδs hσ0 hRdef
  have hres₃ := sf_stepError_arc3 hε hc hh0 hσdec hhdec hsdef hs2 hs0 hrs0 hrs1 hεabs
...
This hypothesis is genuinely `ε ∈ {+1,−1}`-only: at `ε = 0` the coefficient
...
lemma stepError_coeff_ne_zero {ε c : ℝ} (hε : ε = 1 ∨ ε = -1)
  ...
  have hr : 0 < centeredRadius ε c :=
    (centeredRadius_mem_Ioo ε c (hε.imp_right Or.inl) (hc.imp_right Or.inl)).1
