...
/-!
# Space-form reconstruction flow (`K`-generic)

The confined vector field `F_{K,κ,R,δ}(θ, z) = q̂_{K,κ,R,δ}(θ, z)·e^{iθ}` (the
gauge speed clamped to the admissible slab) and its Picard–Lindelöf flow. `K`-
generic transport of `Gluck/Sphere/Flow.lean`; the ODE/Grönwall scaffolding is
structurally model-agnostic, only the fed field carries `K`. The truncated
numerator `1 + K(min ‖z‖ R)²` stays positive for `|K| ≤ 1` once `R < 1`
(automatic in the disk), so the clamp keeps the field globally tame in both
the spherical (`K=+1`) and hyperbolic (`K=−1`) models.
-/
...
`q̂_{K,κ,R,δ}(θ, z) = (1 + K(min ‖z‖ R)²) / (2 · max (κ(θ) − K⟪z, i·e^{iθ}⟫_ℝ) δ)`.
On the admissible set `{‖z‖ ≤ R ∧ δ ≤ κ(θ) − K⟪z, i·e^{iθ}⟫}` both clamps are
inactive and `q̂ = q_{K,κ}` (`truncatedSpeed_eq`). Total function; hypotheses
`|K| ≤ 1`, `R < 1`, `0 < δ` go on the lemmas. -/
noncomputable def truncatedSpeed (K : ℝ) (κ : ℝ → ℝ) (R δ θ : ℝ) (z : ℂ) : ℝ :=
  (1 + K * (min ‖z‖ R) ^ 2) /
    (2 * max (κ θ - K * ⟪z, Complex.I * Complex.exp ((θ : ℂ) * Complex.I)⟫_ℝ) δ)

/-- **Truncated speed agrees on the admissible set.** If `‖z‖ ≤ R` and
`δ ≤ κ(θ) − K⟪z, i·e^{iθ}⟫_ℝ` both clamps are inactive and `q̂ = q_{K,κ}`. -/
lemma truncatedSpeed_eq {K : ℝ} {κ : ℝ → ℝ} {R δ θ : ℝ} {z : ℂ} (hz : ‖z‖ ≤ R)
    (hδ : δ ≤ κ θ - K * ⟪z, Complex.I * Complex.exp ((θ : ℂ) * Complex.I)⟫_ℝ) :
    truncatedSpeed K κ R δ θ z = spaceFormSpeed K κ θ z := by
  unfold truncatedSpeed spaceFormSpeed
  rw [min_eq_left hz, max_eq_left hδ]

/-- **Truncated numerator positivity.** For `|K| ≤ 1`, `0 ≤ R < 1` the clamped
numerator `1 + K(min ‖z‖ R)²` is `≥ 1 − R² > 0`. -/
lemma truncatedNum_pos {K : ℝ} (hK : |K| ≤ 1) {R : ℝ} (hR : 0 ≤ R) (hR1 : R < 1)
    (z : ℂ) : 0 < 1 + K * (min ‖z‖ R) ^ 2 := by
  have hm0 : 0 ≤ min ‖z‖ R := le_min (norm_nonneg _) hR
  have hmR : min ‖z‖ R ≤ R := min_le_right _ _
  have hsq : (min ‖z‖ R) ^ 2 < 1 := by nlinarith
  have hKlow : -1 ≤ K := (abs_le.mp hK).1
  nlinarith [mul_nonneg (by linarith : (0 : ℝ) ≤ K + 1) (sq_nonneg (min ‖z‖ R))]

/-- **Truncated speed is positive**: numerator `> 0` (`truncatedNum_pos`) and
denominator `≥ 2δ > 0`. -/
lemma truncatedSpeed_pos {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ θ : ℝ} {z : ℂ}
    (hR : 0 ≤ R) (hR1 : R < 1) (hδ : 0 < δ) : 0 < truncatedSpeed K κ R δ θ z := by
  have hnum := truncatedNum_pos hK hR hR1 z
  have hden : (0 : ℝ) < 2 *
      max (κ θ - K * ⟪z, Complex.I * Complex.exp ((θ : ℂ) * Complex.I)⟫_ℝ) δ :=
    mul_pos two_pos (hδ.trans_le (le_max_right _ _))
  exact div_pos hnum hden

/-- **Truncated speed is bounded** by `B = (1 + R²)/(2δ)`: for `|K| ≤ 1` the
numerator is `≤ 1 + R²`, and the denominator is `≥ 2δ`. -/
lemma truncatedSpeed_le {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ θ : ℝ} {z : ℂ}
    (hR : 0 ≤ R) (hδ : 0 < δ) :
    truncatedSpeed K κ R δ θ z ≤ (1 + R ^ 2) / (2 * δ) := by
  have hmin0 : (0 : ℝ) ≤ min ‖z‖ R := le_min (norm_nonneg _) hR
  have hminR : min ‖z‖ R ≤ R := min_le_right _ _
  have hKhi : K ≤ 1 := (abs_le.mp hK).2
  have hnum : 1 + K * (min ‖z‖ R) ^ 2 ≤ 1 + R ^ 2 := by
    nlinarith [mul_le_mul hminR hminR hmin0 hR, sq_nonneg (min ‖z‖ R)]
  have hden : 2 * δ ≤ 2 *
      max (κ θ - K * ⟪z, Complex.I * Complex.exp ((θ : ℂ) * Complex.I)⟫_ℝ) δ := by
    have := le_max_right
      (κ θ - K * ⟪z, Complex.I * Complex.exp ((θ : ℂ) * Complex.I)⟫_ℝ) δ
    linarith
...
is `2R`-Lipschitz and bounded by `1 + R²` (for `|K| ≤ 1`), the clamped
denominator is `2`-Lipschitz and `≥ 2δ`. -/
lemma truncatedSpeed_lipschitz {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ : ℝ}
    (hR : 0 ≤ R) (hR1 : R < 1) (hδ : 0 < δ) :
    ∃ L : ℝ≥0, ∀ θ, LipschitzWith L (fun z => truncatedSpeed K κ R δ θ z) := by
...
lemma truncatedSpeed_continuous {K : ℝ} {κ : ℝ → ℝ} {R δ : ℝ} (hκ : Continuous κ)
    (hδ : 0 < δ) :
    Continuous fun p : ℝ × ℂ => truncatedSpeed K κ R δ p.1 p.2 := by
  ...
  have hnum : Continuous fun p : ℝ × ℂ => 1 + K * (min ‖p.2‖ R) ^ 2 :=
    continuous_const.add
      (continuous_const.mul ((continuous_snd.norm.min continuous_const).pow 2))
  have hden : Continuous fun p : ℝ × ℂ =>
      2 * max (κ p.1 - K * ⟪p.2, Complex.I * Complex.exp ((p.1 : ℂ) * Complex.I)⟫_ℝ) δ :=
...
/-- The *truncated reconstruction field* `F_{K,κ,R,δ}(θ, z) = q̂ · e^{iθ}` — the
right-hand side of the truncated reconstruction ODE `z' = F(θ, z)`. -/
noncomputable def truncatedField (K : ℝ) (κ : ℝ → ℝ) (R δ θ : ℝ) (z : ℂ) : ℂ :=
  truncatedSpeed K κ R δ θ z • Complex.exp ((θ : ℂ) * Complex.I)

/-- The field inherits the norm of the speed: `‖F‖ = q̂` since `‖e^{iθ}‖ = 1`
and `q̂ > 0`. -/
lemma norm_truncatedField {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ : ℝ}
    (hR : 0 ≤ R) (hR1 : R < 1) (hδ : 0 < δ) (θ : ℝ) (z : ℂ) :
    ‖truncatedField K κ R δ θ z‖ = truncatedSpeed K κ R δ θ z := by
  rw [truncatedField, norm_smul, Real.norm_eq_abs, Complex.norm_exp_ofReal_mul_I,
    mul_one, abs_of_pos (truncatedSpeed_pos hK hR hR1 hδ)]

/-- The truncated field inherits the uniform-in-`θ` Lipschitz constant of the
truncated speed. -/
lemma truncatedField_lipschitz {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ : ℝ}
    (hR : 0 ≤ R) (hR1 : R < 1) (hδ : 0 < δ) :
    ∃ L : ℝ≥0, ∀ θ, LipschitzWith L (fun z => truncatedField K κ R δ θ z) := by
  obtain ⟨L, hL⟩ := truncatedSpeed_lipschitz hK (κ := κ) hR hR1 hδ
...
lemma truncatedField_continuous {K : ℝ} {κ : ℝ → ℝ} {R δ : ℝ} (hκ : Continuous κ)
    (hδ : 0 < δ) :
    Continuous fun p : ℝ × ℂ => truncatedField K κ R δ p.1 p.2 := by
  unfold truncatedField
...
covers `[0, 2π]`. -/
lemma truncatedField_isPicardLindelof {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ : ℝ}
    (hκ : Continuous κ) (hR : 0 ≤ R) (hR1 : R < 1) (hδ : 0 < δ) (r₀ : ℝ≥0) :
    ∃ a L C : ℝ≥0, IsPicardLindelof (truncatedField K κ R δ)
  ...
  refine ⟨r₀ + (2 * π * B + 1).toNNReal, B.toNNReal, C, ?_, ?_, ?_, ?_⟩
  · exact fun t _ => (hC t).lipschitzOnWith
  · intro x _
    exact ((truncatedField_continuous hκ hδ).comp
      (continuous_id.prodMk continuous_const)).continuousOn
  · intro t _ x _
    rw [norm_truncatedField hK hR hR1 hδ, Real.coe_toNNReal _ hB0, hB]
    exact truncatedSpeed_le hK hR hδ
...
`F_{K,κ,R,δ}` on `[0, 2π]`, jointly continuously. -/
lemma exists_spaceFormFlow {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ : ℝ}
    (hκ : Continuous κ) (hR : 0 ≤ R) (hR1 : R < 1) (hδ : 0 < δ) (r₀ : ℝ≥0) :
    ∃ α : ℂ × ℝ → ℂ,
      (∀ z₀ ∈ Metric.closedBall (0 : ℂ) r₀,
        ...
        ∀ θ ∈ Set.Icc (0 : ℝ) (2 * π),
          HasDerivWithinAt (fun t => α (z₀, t))
            (truncatedField K κ R δ θ (α (z₀, θ))) (Set.Icc 0 (2 * π)) θ) ∧
      ContinuousOn α (Metric.closedBall 0 r₀ ×ˢ Set.Icc 0 (2 * π)) := by
  obtain ⟨a, L, C, hPL⟩ := truncatedField_isPicardLindelof hK hκ hR hR1 hδ r₀
  obtain ⟨α, hα1, hα2⟩ :=
    hPL.exists_forall_mem_closedBall_eq_hasDerivWithinAt_continuousOn
  exact ⟨α, fun z₀ hz₀ => hα1 z₀ hz₀, hα2⟩

open scoped Classical in
/-- **Picard–Lindelöf flow** `Φ = Φ_{K,κ,R,δ,r₀} : ℂ × ℝ → ℂ`: a choice, made
...
noncomputable def spaceFormFlow (K : ℝ) (κ : ℝ → ℝ) (R δ : ℝ) (r₀ : ℝ≥0) :
    ℂ × ℝ → ℂ :=
  if h : |K| ≤ 1 ∧ Continuous κ ∧ 0 ≤ R ∧ R < 1 ∧ 0 < δ then
    Classical.choose (exists_spaceFormFlow h.1 h.2.1 h.2.2.1 h.2.2.2.1 h.2.2.2.2 r₀)
  else Prod.fst

/-- The closing-error endpoint map `z₀ ↦ Φ(z₀, 2π) − z₀`. -/
noncomputable def spaceFormEndpoint (K : ℝ) (κ : ℝ → ℝ) (R δ : ℝ) (r₀ : ℝ≥0)
    (z₀ : ℂ) : ℂ :=
  spaceFormFlow K κ R δ r₀ (z₀, 2 * π) - z₀

/-- **Flow specification.** For `‖z₀‖ ≤ r₀` the flow starts at `z₀` and solves
`z' = F_{K,κ,R,δ}(θ, z)` on `[0, 2π]`. (Transport of `sphericalFlow_spec`.) -/
lemma spaceFormFlow_spec {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ : ℝ}
    (hκ : Continuous κ) (hR : 0 ≤ R) (hR1 : R < 1) (hδ : 0 < δ) (r₀ : ℝ≥0) {z₀ : ℂ}
    (hz₀ : z₀ ∈ Metric.closedBall (0 : ℂ) r₀) :
    spaceFormFlow K κ R δ r₀ (z₀, 0) = z₀ ∧
      ∀ θ ∈ Set.Icc (0 : ℝ) (2 * π),
        HasDerivWithinAt (fun t => spaceFormFlow K κ R δ r₀ (z₀, t))
          (truncatedField K κ R δ θ (spaceFormFlow K κ R δ r₀ (z₀, θ)))
          (Set.Icc 0 (2 * π)) θ := by
  have h : |K| ≤ 1 ∧ Continuous κ ∧ 0 ≤ R ∧ R < 1 ∧ 0 < δ := ⟨hK, hκ, hR, hR1, hδ⟩
...
lemma spaceFormFlow_continuousOn {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ : ℝ}
    (hκ : Continuous κ) (hR : 0 ≤ R) (hR1 : R < 1) (hδ : 0 < δ) (r₀ : ℝ≥0) :
    ContinuousOn (spaceFormFlow K κ R δ r₀)
...
/-- **Flow uniqueness**: any `g` solving `z' = F_{K,κ,R,δ}(θ, z)` on `[0, 2π]`
with `g 0 = z₀`, `‖z₀‖ ≤ r₀`, agrees with `Φ(z₀, ·)`. The field is globally
Lipschitz in space uniformly in time, so ODE uniqueness applies. -/
lemma spaceFormFlow_unique {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ : ℝ}
    (hκ : Continuous κ) (hR : 0 ≤ R) (hR1 : R < 1) (hδ : 0 < δ) (r₀ : ℝ≥0) {z₀ : ℂ}
    (hz₀ : z₀ ∈ Metric.closedBall (0 : ℂ) r₀) {g : ℝ → ℂ}
    (hg : ∀ θ ∈ Set.Icc (0 : ℝ) (2 * π),
      HasDerivWithinAt g (truncatedField K κ R δ θ (g θ)) (Set.Icc 0 (2 * π)) θ)
    (hg0 : g 0 = z₀) :
    Set.EqOn g (fun θ => spaceFormFlow K κ R δ r₀ (z₀, θ)) (Set.Icc 0 (2 * π)) := by
  obtain ⟨C, hC⟩ := truncatedField_lipschitz hK (κ := κ) hR hR1 hδ
  obtain ⟨hf0, hfderiv⟩ := spaceFormFlow_spec hK hκ hR hR1 hδ r₀ hz₀
  have upgrade : ∀ {u : ℝ → ℂ},
      (∀ θ ∈ Set.Icc (0 : ℝ) (2 * π), HasDerivWithinAt u
        (truncatedField K κ R δ θ (u θ)) (Set.Icc 0 (2 * π)) θ) →
...
lemma spaceFormEndpoint_continuousOn {K : ℝ} (hK : |K| ≤ 1) {κ : ℝ → ℝ} {R δ : ℝ}
    (hκ : Continuous κ) (hR : 0 ≤ R) (hR1 : R < 1) (hδ : 0 < δ) (r₀ : ℝ≥0) :
    ContinuousOn (spaceFormEndpoint K κ R δ r₀) (Metric.closedBall 0 r₀) := by
  ...
  exact ((spaceFormFlow_continuousOn hK hκ hR hR1 hδ r₀).comp
