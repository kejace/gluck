...
form, with a *constant* perturbation `ε` (the `gronwallBound` family). This file proves the
classical *integral* form — the Grönwall–Bellman inequality: if a continuous function `u`
satisfies

  `u t ≤ δ + ∫ s in a..t, (r s * u s + g s)`  on `[a, b]`,

with `r, g` continuous and nonnegative, then `u t ≤ exp (∫ s in a..t, r s) * (δ + ∫ s in a..t,
g s)`. The drive `g` is only assumed small in `L¹`, not pointwise — the regime where a
perturbation is integrally small but has large sup-norm, which the constant-drive lemmas cannot
capture. Mathlib's `Analysis/ODE/DiscreteGronwall.lean` already proves the exact discrete
analogue (`discrete_gronwall`: variable coefficients, `ℓ¹` drive); this file is its continuous
counterpart.

The proof is the textbook weighted-primitive argument: the weight
...
We also prove the trajectory-comparison consequence for ODEs, the `L¹`-drive analogue of
`dist_le_of_approx_trajectories_ODE`: two approximate solutions of a `K`-Lipschitz field with
time-dependent error bounds `εf, εg` satisfy
...
private lemma hasDerivAt_primitive_of_continuousOn {a b : ℝ} {f : ℝ → ℝ}
  ...
  have hint : IntervalIntegrable f MeasureTheory.volume a t :=
    (hf.mono (by
      ...
      exact Icc_subset_Icc_right ht.2.le)).intervalIntegrable
...
private lemma hasDerivWithinAt_primitive_Ici {a b : ℝ} {f : ℝ → ℝ}
  ...
  have hcont : ContinuousWithinAt f (Ioi t) t :=
...
theorem le_exp_integral_mul_of_le_add_intervalIntegral {a b δ : ℝ} (hab : a ≤ b)
  ...
  have hvc : ContinuousOn v (Icc a b) := by
    ...
    exact Real.continuous_exp.comp_continuousOn hPc.neg
  ...
  have h1 : Real.exp (-P t) * U t ≤ δ + G t := by
    ...
    simp only [hv] at hva
    linarith
  have h2 : U t ≤ Real.exp (P t) * (δ + G t) := by
    have h3 := mul_le_mul_of_nonneg_left h1 (Real.exp_nonneg (P t))
...
theorem dist_le_of_approx_trajectories_ODE_of_integral_bound
  ...
  -- Step 1: the fencing bound with an `η`-bump making the boundary inequality strict.
  have key : ∀ η, 0 < η → dist (f t) (g t) ≤ Real.exp (K * (t - a))
    ...
    have hBderiv : ∀ x ∈ Ico a b, HasDerivWithinAt B (B' x) (Ici x) x := by
      ...
      have h1 : HasDerivAt (fun τ : ℝ => Real.exp (K * (τ - a)))
          (Real.exp (K * (x - a)) * K) x := h0.exp
      ...
      exact h1.hasDerivWithinAt.mul h2
...
theorem dist_le_of_trajectories_ODE_of_integral_bound
  ...
  have h := dist_le_of_approx_trajectories_ODE_of_integral_bound
    ...
    continuousOn_const (fun s _ => le_refl 0) hε hε0 ha t ht
  simpa using h
