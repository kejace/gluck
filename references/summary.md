...
| `n-gons.pdf` / `n-gons-applicability.md` | Bachmann–Schmidt, *n-gons* (1975). Affine linear algebra of cyclic vertex tuples: shifts, circulant operators, averaging projections, kernels/images, and root-of-unity components. **Not metric geometry:** its “cyclic classes” are shift-equivariant linear classes, not Euclidean concyclicity. Useful as the design reference for a future `Gluck.Discrete.CyclicLinear` cleanup, especially `centralSym`, difference/zero-sum reconstruction, and repeated `ZMod` shift algebra; it does not prove any Dahlberg disk or curvature lemma. | OCR is usable with `pdftotext -layout n-gons.pdf -`; consult the companion Markdown note for the proposed cleanup API, call-site map, sequencing, and non-goals. |
